...
hardware::SynchronizationEvent hardware::transfer::OclCopy::load(const hardware::buffers::Buffer* orig,
                                                                 ...
                                                                 const hardware::SynchronizationEvent& event)
{
    ...
    load_event = copyDataRect(device, transfer_buffer, orig, transfer_buffer_origin, src_origin, region, 0, 0,
                              src_row_pitch, src_slice_pitch, {load_event, dump_event, event});
    device->flush();
...
hardware::SynchronizationEvent hardware::transfer::OclCopy::dump(const hardware::buffers::Buffer* dest,
                                                                 ...
                                                                 const hardware::SynchronizationEvent& event)
{
    ...
    dump_event = copyDataRect(device, dest, transfer_buffer, dest_origin, transfer_buffer_origin, region,
                              dest_row_pitch, dest_slice_pitch, 0, 0, {load_event, dump_event, event});
    device->flush();
...
hardware::buffers::Buffer* hardware::transfer::OclCopy::get_transfer_buffer(const size_t* const region)
{
    ...
    if (!(bool)transfer_buffer_handle) {
        transfer_buffer_handle = std::unique_ptr<hardware::buffers::Buffer>(
            new hardware::buffers::Buffer(required_buffer_size, get_src_device(), false));
    }
