...
hardware::transfer::AsyncOclCopy::AsyncOclCopy(hardware::Device* const from, hardware::Device* const to,
...
hardware::SynchronizationEvent hardware::transfer::AsyncOclCopy::load(const hardware::buffers::Buffer* orig,
                                                                      ...
                                                                      const hardware::SynchronizationEvent& event)
{
    ...
    load_event = copyDataRect(get_src_device(), transfer_buffer, orig, transfer_buffer_origin, src_origin, region, 0, 0,
                              src_row_pitch, src_slice_pitch,
                              {load_event, transfer_event, event, back_migration_event});
...
hardware::SynchronizationEvent hardware::transfer::AsyncOclCopy::transfer()
{
    ...
    // transfer data to destination device
    {
        ...
        cl_int clerr = clEnqueueCopyBuffer(transfer_queue, *src_cache->get_cl_buffer(), *dest_cache->get_cl_buffer(), 0,
                                           0, active_size, events.size(), events_p, &raw_transfer_event);
        if (clerr) {
...
#if CL_VERSION_1_2
    {
        ...
        cl_int clerr = clEnqueueMigrateMemObjects(back_migration_queue, 1, src_cache->get_cl_buffer(),
                                                  CL_MIGRATE_MEM_OBJECT_CONTENT_UNDEFINED, events.size(), events_p,
                                                  &raw_back_migration_event);
        if (clerr) {
...
#    pragma message "clEnqueueMigrateMemObjects is not available on your system. " \
...
hardware::SynchronizationEvent hardware::transfer::AsyncOclCopy::dump(const hardware::buffers::Buffer* dest,
                                                                      ...
                                                                      const hardware::SynchronizationEvent& event)
{
    ...
    // the transfer may neither overlap with a dump or a load, as both use the transfer buffer
    const size_t transfer_buffer_origin[] = {0, 0, 0};
    dump_event = copyDataRect(get_dest_device(), dest, transfer_buffer, dest_origin, transfer_buffer_origin, region,
...
hardware::buffers::Buffer* hardware::transfer::AsyncOclCopy::get_src_cache(size_t buffer_size)
{
    ...
    if (!(bool)transfer_buffer_handle) {
        transfer_buffer_handle = std::unique_ptr<hardware::buffers::Buffer>(
...
hardware::buffers::Buffer* hardware::transfer::AsyncOclCopy::get_dest_cache(size_t const buffer_size)
{
    ...
    if (!(bool)transfer_buffer_handle) {
        transfer_buffer_handle = std::unique_ptr<hardware::buffers::Buffer>(
            new hardware::buffers::Buffer(buffer_size, get_dest_device(), false));
    }
    return transfer_buffer_handle.get();
}
