...
// in that case it is unavailable
    ...
    try {
        ...
        logger.debug() << "Standard Transfer Performance: " << standard_performance
                       << " GB/s - DirectGMA Performance: " << dgma_performance << " GB/s";

        if (standard_performance > dgma_performance) {
            logger.warn() << "DirectGMA performs badly from " << src->getGridPos() << " to " << dest->getGridPos()
                          << ". Falling back to standard transfer methods.";
            return std::move(standard_transfer);
        } else {
        ...
        logger.warn() << "DirectGMA is unavailable from " << src->getGridPos() << " to " << dest->getGridPos()
                      << ". Falling back to standard transfer methods.";
        return std::move(standard_transfer);
    }
#endif
