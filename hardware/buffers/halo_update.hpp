...
namespace hardware {
    ...
    namespace buffers {
        ...
        void update_halo(std::vector<BUFFER*> buffers, const hardware::System& system, const float ELEMS_PER_SITE = 1.);
        /**
         * Update the halo of the given buffers.
         */
        template<typename T, class BUFFER>
        void
        update_halo_soa(std::vector<BUFFER*> buffers, const hardware::System& system, const float ELEMS_PER_SITE = 1.,
                        const unsigned CHUNKS_PER_LANE = 1, unsigned reqd_width = 0);
        /**
         ...
         * Therefore commands using the default queue of this buffer can safely operate on the non-halo / own elements
         * of this buffer if queued after this call.
         *
         * Note that depending on the exact transfer method used this might already transfer data between devices!
         */
        template<typename T, class BUFFER>
        void initialize_update_halo_soa(std::vector<BUFFER*> buffers, const hardware::System& system,
                                        const float ELEMS_PER_SITE = 1., const unsigned CHUNKS_PER_LANE = 1,
                                        unsigned reqd_width = 0);
        /**
         ...
         */
        template<typename T, class BUFFER>
        void finalize_update_halo_soa(std::vector<BUFFER*> buffers, const hardware::System& system,
                                      const float ELEMS_PER_SITE = 1., const unsigned CHUNKS_PER_LANE = 1,
                                      unsigned reqd_width = 0);

        template<typename BUFFER>
        static hardware::SynchronizationEvent
        extract_boundary(hardware::Transfer* transfer, const BUFFER* buffer, size_t in_lane_offset,
                         size_t HALO_CHUNK_ELEMS, const float ELEMS_PER_SITE, const unsigned CHUNKS_PER_LANE,
                         const hardware::SynchronizationEvent& event);
        template<typename BUFFER>
        static hardware::SynchronizationEvent
        send_halo(hardware::Transfer* transfer, const BUFFER* buffer, size_t in_lane_offset, size_t HALO_CHUNK_ELEMS,
                  const float ELEMS_PER_SITE, const unsigned CHUNKS_PER_LANE,
                  const hardware::SynchronizationEvent& event);
...
template<typename T, class BUFFER>
void hardware::buffers::update_halo(std::vector<BUFFER*> buffers, const hardware::System& system,
                                    const float ELEMS_PER_SITE)
{
    ...
    if (num_buffers > 1) {
        ...
        for (size_t i = 0; i < num_buffers; ++i) {
            ...
            // its storage location is wrapped around to be the last chunk of data in our buffer, that is after local
            // data and upper halo
...
template<typename BUFFER>
static hardware::SynchronizationEvent
hardware::buffers::extract_boundary(hardware::Transfer* transfer, const BUFFER* buffer, size_t in_lane_offset,
                                    size_t HALO_CHUNK_ELEMS, const float ELEMS_PER_SITE, const unsigned CHUNKS_PER_LANE,
                                    const hardware::SynchronizationEvent& event)
{
    logger.debug() << "Extracting boundary. Offset: " << in_lane_offset
                   << " - Elements per chunk: " << HALO_CHUNK_ELEMS;
...
template<typename BUFFER>
static hardware::SynchronizationEvent
hardware::buffers::send_halo(hardware::Transfer* transfer, const BUFFER* buffer, size_t in_lane_offset,
                             size_t HALO_CHUNK_ELEMS, const float ELEMS_PER_SITE, const unsigned CHUNKS_PER_LANE,
                             const hardware::SynchronizationEvent& event)
{
    ...
    namespace buffers {
        ...
        class ProxyBufferCache {
          ...
          public:
            ...
            const std::vector<DeviceAccessibleMemory*>&
            getBuffers(size_t rows, size_t bytes, const std::vector<hardware::Device*>& devices);
        };
...
struct UpdateHaloSOAhelper {
    UpdateHaloSOAhelper(std::vector<BUFFER*> const& buffers, const hardware::System& system, const float ELEMS_PER_SITE,
                        const unsigned CHUNKS_PER_LANE, unsigned reqd_width)
    {
        const auto main_device = buffers[0]->get_device();
        LatticeGrid lG(main_device->getGridSize());
...
template<typename T, class BUFFER>
void hardware::buffers::initialize_update_halo_soa(std::vector<BUFFER*> buffers, const hardware::System& system,
                                                   const float ELEMS_PER_SITE, const unsigned CHUNKS_PER_LANE,
                                                   unsigned reqd_width)
{
    ...
    if (num_buffers > 1) {
        ...
        for (size_t i = 0; i < num_buffers; ++i) {
            ...
            extract_boundary(up_transfer, buffer, helper.vol4d_local - helper.reqd_halo_chunk_elems,
                             helper.reqd_halo_chunk_elems, ELEMS_PER_SITE, CHUNKS_PER_LANE, SynchronizationEvent());
            auto const down_transfer = system.get_transfer(i, lower_grid_neighbour(i, helper.grid_size), DOWN_TRANSFER);
            extract_boundary(down_transfer, buffer, 0, helper.reqd_halo_chunk_elems, ELEMS_PER_SITE, CHUNKS_PER_LANE,
...
template<typename T, class BUFFER>
void hardware::buffers::finalize_update_halo_soa(std::vector<BUFFER*> buffers, const hardware::System& system,
                                                 const float ELEMS_PER_SITE, const unsigned CHUNKS_PER_LANE,
                                                 unsigned reqd_width)
{
    const size_t num_buffers = buffers.size();
    if (num_buffers > 1) {
        ...
        for (size_t i = 0; i < num_buffers; ++i) {
            ...
            send_halo(up_transfer, buffer,
                      helper.vol4d_local + 2 * helper.total_halo_chunk_elems - helper.reqd_halo_chunk_elems,
                      helper.reqd_halo_chunk_elems, ELEMS_PER_SITE, CHUNKS_PER_LANE, SynchronizationEvent());
            // our upper halo is the lower bounary of our upper neighbour, it's stored right after our local data
            auto const down_transfer = system.get_transfer(upper_grid_neighbour(i, helper.grid_size), i, DOWN_TRANSFER);
            send_halo(down_transfer, buffer, helper.vol4d_local, helper.reqd_halo_chunk_elems, ELEMS_PER_SITE,
                      CHUNKS_PER_LANE, SynchronizationEvent());
        }
    }
}

template<typename T, class BUFFER>
void hardware::buffers::update_halo_soa(std::vector<BUFFER*> buffers, const hardware::System& system,
                                        const float ELEMS_PER_SITE, const unsigned CHUNKS_PER_LANE, unsigned reqd_width)
{
