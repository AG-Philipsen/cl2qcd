    ...
    namespace buffers {
        ...
        class Plain : public Buffer {
          public:
             ...
             */
            Plain(const size_t elems, const Device* device, bool place_on_host = false,
                  cl_mem_flags additional_flags = 0)
             ...
             */
            hardware::SynchronizationEvent load_async(const T* ptr) const { return Buffer::load_async(ptr); }
             ...
             * returns true on is_finished().
             */
            hardware::SynchronizationEvent dump_async(T* ptr) const { return Buffer::dump_async(ptr); }

            /**
             * Get the size of the buffer in elements
             */
            inline size_t get_elements() const noexcept { return elements; }

            /**
             * Get data from another buffer.
             * If the whole buffer should be copied use copyData(dest, src) instead!
             */
            void copyDataBlock(const Plain<T>* orig, const size_t dest_offset, const size_t src_offset = 0,
                               size_t elems = 0) const
            {
                if (!elems) {
