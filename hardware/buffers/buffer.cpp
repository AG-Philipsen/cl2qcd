...
struct MemObjectAllocationTracer {
    ...
    ~MemObjectAllocationTracer() { device->markMemReleased(host, bytes); }
};

hardware::buffers::Buffer::Buffer(const size_t bytes, const hardware::Device* device, const bool place_on_host,
                                  const cl_mem_flags extra_flags)
    : bytes(bytes), cl_buffer(allocateBuffer(bytes, device->context, place_on_host, extra_flags)), device(device)
{
    ...
    cl_int err = clSetMemObjectDestructorCallback(cl_buffer, memObjectReleased,
                                                  new MemObjectAllocationTracer(bytes, place_on_host, device));
    if (err) {
...
static cl_mem
allocateBuffer(const size_t bytes, const cl_context context, const bool place_on_host, const cl_mem_flags extra_flags)
{
    cl_int err;
...
void hardware::buffers::Buffer::load(const void* array, size_t bytes, const size_t offset) const
{
    if (bytes == 0) {
        ...
        if (bytes + offset > this->bytes) {
            logger.error() << "Writing outside buffer. Bytes: " << bytes << " Offset: " << offset
                           << " Buffer size: " << this->bytes;
...
void hardware::buffers::Buffer::dump(void* array, size_t bytes, const size_t offset) const
{
    if (bytes == 0) {
        ...
        if (bytes + offset > this->bytes) {
            logger.error() << "Reading outside buffer. Bytes: " << bytes << " Offset: " << offset
                           << " Buffer size: " << this->bytes;
...
void hardware::buffers::Buffer::load_rect(const void* src, const size_t* buffer_origin, const size_t* host_origin,
                                          const size_t* region, size_t buffer_row_pitch, size_t buffer_slice_pitch,
                                          size_t host_row_pitch, size_t host_slice_pitch) const
{
    cl_int err = clEnqueueWriteBufferRect(*device, cl_buffer, CL_TRUE, buffer_origin, host_origin, region,
                                          buffer_row_pitch, buffer_slice_pitch, host_row_pitch, host_slice_pitch, src,
                                          0, nullptr, nullptr);
    if (err) {
...
void hardware::buffers::Buffer::dump_rect(void* src, const size_t* buffer_origin, const size_t* host_origin,
                                          const size_t* region, size_t buffer_row_pitch, size_t buffer_slice_pitch,
                                          size_t host_row_pitch, size_t host_slice_pitch) const
{
    cl_int err = clEnqueueReadBufferRect(*device, cl_buffer, CL_TRUE, buffer_origin, host_origin, region,
                                         buffer_row_pitch, buffer_slice_pitch, host_row_pitch, host_slice_pitch, src, 0,
                                         nullptr, nullptr);
    if (err) {
        throw hardware::OpenclException(err, "clEnqueueReadBufferRect", __FILE__, __LINE__);
    }
...
hardware::buffers::Buffer::load_rectAsync(const void* src, const size_t* buffer_origin, const size_t* host_origin,
                                          const size_t* region, size_t buffer_row_pitch, size_t buffer_slice_pitch,
                                          ...
                                          const hardware::SynchronizationEvent& event) const
{
    ...
    cl_event event_cl;
    cl_int err = clEnqueueWriteBufferRect(*device, cl_buffer, CL_FALSE, buffer_origin, host_origin, region,
                                          buffer_row_pitch, buffer_slice_pitch, host_row_pitch, host_slice_pitch, src,
                                          num_wait_events, wait_event, &event_cl);
    if (err) {
        throw hardware::OpenclException(err, "clEnqueueReadBufferRect", __FILE__, __LINE__);
    }
...
hardware::SynchronizationEvent
hardware::buffers::Buffer::dump_rectAsync(void* dest, const size_t* buffer_origin, const size_t* host_origin,
                                          const size_t* region, size_t buffer_row_pitch, size_t buffer_slice_pitch,
                                          size_t host_row_pitch, size_t host_slice_pitch,
                                          const hardware::SynchronizationEvent& event) const
{
    ...
    cl_event event_cl;
    cl_int err = clEnqueueReadBufferRect(*device, cl_buffer, CL_FALSE, buffer_origin, host_origin, region,
                                         buffer_row_pitch, buffer_slice_pitch, host_row_pitch, host_slice_pitch, dest,
                                         num_wait_events, wait_event, &event_cl);
    if (err) {
        throw hardware::OpenclException(err, "clEnqueueReadBufferRect", __FILE__, __LINE__);
    }
    ...
    if (this->bytes != orig->bytes) {
        ...
        if (this->bytes == 16 && (dev_name == "Cypress" || dev_name == "Cayman")) {
            ...
            logger.debug() << "Using default OpenCL buffer copy method for " << this->bytes << " bytes on " << dev_name
                           << '.';
            int err = clEnqueueCopyBuffer(device->get_queue(), orig->cl_buffer, this->cl_buffer, 0, 0, this->bytes, 0,
                                          nullptr, nullptr);
            if (err) {
...
void hardware::buffers::Buffer::copyDataBlock(const Buffer* orig, const size_t dest_offset, const size_t src_offset,
                                              const size_t bytes) const
{
    ...
    if (this->bytes < dest_offset + bytes || orig->bytes < src_offset + bytes) {
        ...
        int err = clEnqueueCopyBuffer(device->get_queue(), orig->cl_buffer, this->cl_buffer, src_offset, dest_offset,
...
void hardware::buffers::Buffer::clear() const
{
    ...
    if (sizeof(hmc_complex_zero) % bytes) {
        ...
        cl_int err = clEnqueueFillBuffer(*device, cl_buffer, &hmc_complex_zero, sizeof(hmc_complex_zero), 0, bytes, 0,
                                         nullptr, nullptr);
        if (err) {
            throw hardware::OpenclException(err, "clEnqueueFillBuffer", __FILE__, __LINE__);
        }
...
hardware::SynchronizationEvent
hardware::buffers::copyDataRect(const hardware::Device* device, const hardware::buffers::Buffer* dest,
                                const hardware::buffers::Buffer* orig, const size_t* dest_origin,
                                const size_t* src_origin, const size_t* region, size_t dest_row_pitch,
                                size_t dest_slice_pitch, size_t src_row_pitch, size_t src_slice_pitch,
                                const std::vector<hardware::SynchronizationEvent>& events)
{
    auto const raw_events              = get_raw_events(events);
    cl_event const* const raw_events_p = raw_events.size() > 0 ? raw_events.data() : nullptr;

    cl_event event_cl;
    cl_int err = clEnqueueCopyBufferRect(*device, *orig->get_cl_buffer(), *dest->get_cl_buffer(), src_origin,
                                         dest_origin, region, src_row_pitch, src_slice_pitch, dest_row_pitch,
                                         dest_slice_pitch, raw_events.size(), raw_events_p, &event_cl);
    if (err) {
...
#ifdef CL_VERSION_1_2
void hardware::buffers::Buffer::migrate(hardware::Device* device,
                                        const std::vector<hardware::SynchronizationEvent>& events,
...
hardware::buffers::MappedBufferHandle::MappedBufferHandle(cl_mem buf, cl_command_queue queue, void* mapped_ptr,
