...
struct StaggeredFermionsSourceTestParameters : public PrngSpinorStaggeredTestParameters {
    StaggeredFermionsSourceTestParameters(const LatticeExtents lE, common::sourcecontents sC, common::sourcetypes sT,
                                          const int iterations)
        : TestParameters(lE, 10e-2)
        , PrngSpinorStaggeredTestParameters(lE, iterations, false)
        , sourcecontent(sC)
        , sourcetype(sT){};  // In calling the TestParameters ctor, the testPrecision is reduced, so as related tests
                             // can pass with a reasonable number of iterations!
...
ReferenceValues calculateReferenceValues_volumeSource(const StaggeredFermionsSourceTestParameters tP)
{
    ...
    if (tP.sourcecontent == common::sourcecontents::one) {
        ...
        logger.warn() << "mean = 3 * " << calculateSpinorfieldSize(tP.latticeExtents) << " / (6 * "
                      << calculateEvenOddSpinorfieldSize(tP.latticeExtents) << ") = " << mean;
    } else if (tP.sourcecontent == common::sourcecontents::gaussian or tP.sourcecontent == common::sourcecontents::z4 or
               tP.sourcecontent == common::sourcecontents::z2) {
        mean = 0.;
    } else
        mean = 0.123456;

    variance                   = sqrt(normalizeEvenOdd(((0. - mean) * (0. - mean) * 3 + (1. - mean) * (1. - mean) * 3) *
                                         calculateEvenOddSpinorfieldSize(tP.latticeExtents),
                                     tP.latticeExtents));
...
ReferenceValues calculateReferenceValues_pointSource(const StaggeredFermionsSourceTestParameters tP)
{
    ...
    variance = sqrt(normalizeEvenOdd((0. - mean) * (0. - mean) *
                                             ((calculateEvenOddSpinorfieldSize(tP.latticeExtents) - 1) * 6 + 5) +
                                         (1. - mean) * (1. - mean),
                                     tP.latticeExtents));
...
struct SourceTester : public PrngSpinorStaggeredTester {
    SourceTester(const std::string kernelName, const ParameterCollection pC,
                 const StaggeredFermionsSourceTestParameters& tP, const ReferenceValues rV)
        : PrngSpinorStaggeredTester(kernelName, pC, tP, calculateEvenOddSpinorfieldSize(tP.latticeExtents), rV)
...
struct VolumeSourceTester : public SourceTester {
    VolumeSourceTester(const ParameterCollection pC, const StaggeredFermionsSourceTestParameters tP,
...
struct PointSourceTester : public SourceTester {
    PointSourceTester(const ParameterCollection pC, const StaggeredFermionsSourceTestParameters tP,
                      const int numberOfElements)
...
void callSourceTest(const LatticeExtents lE, const common::sourcecontents sourcecontent,
                    const common::sourcetypes sourcetype, const int iterations)
{
    ...
    hardware::code::OpenClKernelParametersMockupForStaggeredSourceTests
        kernelParameters(parametersForThisTest.latticeExtents, parametersForThisTest.sourcecontent,
                         parametersForThisTest.sourcetype);
...
struct psCorrelatorStaggeredTester : public EvenOddSpinorStaggeredTester {
    psCorrelatorStaggeredTester(const KernelIdentifier kI, const ParameterCollection pC, const ReferenceValues rV,
                                const CorrelatorStaggeredTestParameters tP)
    ...
    ~psCorrelatorStaggeredTester() { CorrelatorResult->dump(&kernelResult.at(0)); }
...
template<class TesterClass>
void callCorrelatorTest(const KernelIdentifier kI, const LatticeExtents lE, const CorrelatorDirection cD,
                        const SpinorFillTypes sF, const ReferenceValues rV)
{
    ...
    hardware::code::OpenClKernelParametersMockupForStaggeredCorrelators
        kernelParameters(parametersForThisTest.latticeExtents, parametersForThisTest.kappa,
                         parametersForThisTest.direction);
    ParameterCollection parameterCollection{hardwareParameters, kernelParameters};
    TesterClass(kI, parameterCollection, rV, parametersForThisTest);
}

void testpsStaggeredCorrelator(const LatticeExtents lE, const CorrelatorDirection cD, const SpinorFillTypes sF,
...
BOOST_AUTO_TEST_SUITE(SRC_VOLUME)
    ...
    BOOST_AUTO_TEST_CASE(SRC_VOLUME_1) { testVolumeSource(LatticeExtents{ns4, nt4}, common::sourcecontents::one, 500); }

    BOOST_AUTO_TEST_CASE_EXPECTED_FAILURES(SRC_VOLUME_2, 2)

    BOOST_AUTO_TEST_CASE(SRC_VOLUME_2) { testVolumeSource(LatticeExtents{ns4, nt4}, common::sourcecontents::z4, 1000); }
    ...
    BOOST_AUTO_TEST_CASE(SRC_VOLUME_4) { testVolumeSource(LatticeExtents{ns4, nt4}, common::sourcecontents::z2, 1000); }

BOOST_AUTO_TEST_SUITE_END()

BOOST_AUTO_TEST_SUITE(SRC_POINT)

    BOOST_AUTO_TEST_CASE(SRC_POINT_1) { testPointSource(LatticeExtents{ns4, nt4}, common::sourcecontents::one, 3); }
...
BOOST_AUTO_TEST_SUITE(CORRELATOR_PS_T)
    ...
    {
        testpsStaggeredCorrelator(LatticeExtents{ns4, nt4}, CorrelatorDirection::temporal,
                                  SpinorFillTypes{SpinorFillType::zero, SpinorFillType::zero}, ReferenceValues(nt4, 0));
    }

    BOOST_AUTO_TEST_CASE(one_one)
    {
        testpsStaggeredCorrelator(LatticeExtents{ns4, nt4}, CorrelatorDirection::temporal,
                                  SpinorFillTypes{SpinorFillType::one, SpinorFillType::one}, ReferenceValues(nt4, 3));
    }

    BOOST_AUTO_TEST_CASE(one_ascendingReal)
    {
        testpsStaggeredCorrelator(LatticeExtents{ns4, nt4}, CorrelatorDirection::temporal,
                                  SpinorFillTypes{SpinorFillType::one, SpinorFillType::ascendingReal},
                                  ReferenceValues(nt4, 8.5));
    }
    ...
    BOOST_AUTO_TEST_CASE(one_ascendingComplex)
    {
        testpsStaggeredCorrelator(LatticeExtents{ns4, nt4}, CorrelatorDirection::temporal,
                                  ...
                                  ReferenceValues(nt4, 47));
    }

    BOOST_AUTO_TEST_CASE(ascendingReal_ascendingReal)
    {
        testpsStaggeredCorrelator(LatticeExtents{ns4, nt4}, CorrelatorDirection::temporal,
                                  SpinorFillTypes{SpinorFillType::ascendingReal, SpinorFillType::ascendingReal},
                                  ReferenceValues(nt4, 14));
    }

    BOOST_AUTO_TEST_CASE(ascendingReal_ascendingComplex)
    {
        testpsStaggeredCorrelator(LatticeExtents{ns8, nt4}, CorrelatorDirection::temporal,
                                  SpinorFillTypes{SpinorFillType::ascendingReal, SpinorFillType::ascendingComplex},
                                  ReferenceValues(nt4, 52.5));
    }

    BOOST_AUTO_TEST_CASE(ascendingComplex_ascendingReal)
    {
        testpsStaggeredCorrelator(LatticeExtents{ns8, nt4}, CorrelatorDirection::temporal,
                                  SpinorFillTypes{SpinorFillType::ascendingComplex, SpinorFillType::ascendingReal},
                                  ReferenceValues(nt4, 52.5));
    }
