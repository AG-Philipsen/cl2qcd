...
void hardware::code::Gaugefield::fill_kernels()
{
    ...
    if (kernelParameters->getUseSmearing() == true) {
        stout_smear = createKernel("stout_smear") << basic_opencl_code << "operations_gaugemomentum.cl"
                                                  << "stout_smear.cl";
    }
...
void hardware::code::Gaugefield::plaquette_device(const hardware::buffers::SU3* gf,
                                                  const hardware::buffers::Plain<hmc_float>* plaq,
                                                  const hardware::buffers::Plain<hmc_float>* tplaq,
                                                  const hardware::buffers::Plain<hmc_float>* splaq) const
{
...
void hardware::code::Gaugefield::rectangles_device(const hardware::buffers::SU3* gf,
                                                   const hardware::buffers::Plain<hmc_float>* rect) const
{
...
void hardware::code::Gaugefield::polyakov_device(const hardware::buffers::SU3* gf,
                                                 const hardware::buffers::Plain<hmc_complex>* pol) const
{
    if (!polyakov) {
...
void hardware::code::Gaugefield::polyakov_md_local_device(const hardware::buffers::Plain<Matrixsu3>* partial_results,
                                                          const hardware::buffers::SU3* gf) const
{
    if (!polyakov_md_local) {
...
void hardware::code::Gaugefield::polyakov_md_merge_device(const hardware::buffers::Plain<Matrixsu3>* partial_results,
                                                          const cl_uint num_slices,
                                                          const hardware::buffers::Plain<hmc_complex>* pol) const
{
    if (!polyakov_md_merge) {
        throw std::logic_error("This function can only called in multi-device environments.");
...
void hardware::code::Gaugefield::gaugeobservables_rectangles(const hardware::buffers::SU3* gf,
                                                             hmc_float* rect_out) const
{
    ...
    rect.dump(rect_out);
    // NOTE: the rectangle value has not been normalized since it is mostly used for the HMC where one needs the
    // absolute value
}

void hardware::code::Gaugefield::stout_smear_device(const hardware::buffers::SU3* in,
                                                    const hardware::buffers::SU3* out) const
{
    // query work-sizes for kernel
    size_t ls, gs;
    cl_uint num_groups;
...
void hardware::code::Gaugefield::get_work_sizes(const cl_kernel kernel, size_t* ls, size_t* gs,
...
uint64_t hardware::code::Gaugefield::get_flop_size(const std::string& in) const
{
    ...
    if (in == "polyakov") {
        // this kernel performs NTIME -1 su3matrix-multiplications, takes a complex trace and adds these real values
        // over VOLSPACE
        return VOLSPACE *
               ((kernelParameters->getNt() - 1) * getFlopSu3MatrixTimesSu3Matrix() + getFlopSu3MatrixTrace());
    }
    if (in == "polyakov_reduction") {
...
void hardware::code::Gaugefield::convertGaugefieldToSOA_device(const hardware::buffers::SU3* out,
                                                               const hardware::buffers::Plain<Matrixsu3>* in) const
{
    if (!out->is_soa()) {
...
void hardware::code::Gaugefield::convertGaugefieldFromSOA_device(const hardware::buffers::Plain<Matrixsu3>* out,
                                                                 const hardware::buffers::SU3* in) const
{
    if (!in->is_soa()) {
...
hardware::code::Gaugefield::Gaugefield(const hardware::code::OpenClKernelParametersInterface& kernelParameters,
                                       const hardware::Device* device)
    : Opencl_Module(kernelParameters, device), stout_smear(0), rectangles(0), rectangles_reduction(0)
{
    fill_kernels();
}
