...
struct TestParametersRealUpdate : public RealTestParameters {
    ...
    TestParametersRealUpdate(const double a, const double b, const double c, const double d, const double e,
                             const double f = 1)
...
{
    return ReferenceValues{(tP.alpha * tP.beta * tP.gamma) / (tP.delta * tP.epsilon * (tP.beta - tP.alpha) +
                                                              tP.beta * tP.gamma * (1. - tP.zeta * tP.delta))};
}

struct RealTester : public KernelTester {
    RealTester(std::string kernelName, const ParameterCollection pC, const RealTestParameters tP,
    ...
    virtual ~RealTester() { result->dump(&kernelResult.at(0)); }
...
{
    TestParametersRealUpdate parametersForThisTest(valuesIn.at(0), valuesIn.at(1), valuesIn.at(2), valuesIn.at(3),
                                                   valuesIn.at(4), valuesIn.at(5));
    hardware::HardwareParametersMockup hardwareParameters(parametersForThisTest.ns, parametersForThisTest.nt);
    hardware::code::OpenClKernelParametersMockup kernelParameters(parametersForThisTest.ns, parametersForThisTest.nt);
    ParameterCollection parameterCollection{hardwareParameters, kernelParameters};
    TesterClass(parameterCollection, parametersForThisTest);
...
BOOST_AUTO_TEST_SUITE(PRODUCT)
    ...
    BOOST_AUTO_TEST_CASE(PRODUCT_1) { testProduct(0., 0.); }

    BOOST_AUTO_TEST_CASE(PRODUCT_2) { testProduct(-nonTrivialParameter, nonTrivialParameter); }

BOOST_AUTO_TEST_SUITE_END()

BOOST_AUTO_TEST_SUITE(RATIO)

    BOOST_AUTO_TEST_CASE(RATIO_1) { testRatio(2. * nonTrivialParameter, -nonTrivialParameter); }

BOOST_AUTO_TEST_SUITE_END()

BOOST_AUTO_TEST_SUITE(SUM)

    BOOST_AUTO_TEST_CASE(SUM_1) { testSum(nonTrivialParameter, -nonTrivialParameter); }

    BOOST_AUTO_TEST_CASE(SUM_2) { testSum(3. * nonTrivialParameter, -5. * nonTrivialParameter); }

BOOST_AUTO_TEST_SUITE_END()

BOOST_AUTO_TEST_SUITE(DIFFERENCE)

    BOOST_AUTO_TEST_CASE(DIFFERENCE_1) { testDifference(nonTrivialParameter, nonTrivialParameter); }

    BOOST_AUTO_TEST_CASE(DIFFERENCE_2) { testDifference(3. * nonTrivialParameter, -5. * nonTrivialParameter); }

BOOST_AUTO_TEST_SUITE_END()

BOOST_AUTO_TEST_SUITE(SET_ELEMENT)

    BOOST_AUTO_TEST_CASE(SET_ELEMENT_1) { testAccess<RealSetVectorElementTester>(); }

    BOOST_AUTO_TEST_CASE(SET_ELEMENT_2) { testAccess<RealSetVectorElementTester>(); }

BOOST_AUTO_TEST_SUITE_END()

BOOST_AUTO_TEST_SUITE(ACCESS_ELEMENT)

    BOOST_AUTO_TEST_CASE(ACCESS_ELEMENT_1) { testAccess<RealAccessVectorElementTester>(); }

    BOOST_AUTO_TEST_CASE(ACCESS_ELEMENT_2) { testAccess<RealAccessVectorElementTester>(); }
