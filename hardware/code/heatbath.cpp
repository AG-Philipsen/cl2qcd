...
{
    ClSourcePackage sources = get_basic_sources()
                              << get_device()->getPrngCode()->get_sources() << "operations_geometry.cl"
    ...
    heatbath_even = createKernel("heatbath_even") << sources << "operations_heatbath.cl"
                                                  << "heatbath_even.cl";
    heatbath_odd = createKernel("heatbath_odd") << sources << "operations_heatbath.cl"
                                                << "heatbath_odd.cl";

    logger.debug() << "Create overrelax kernels...";
    overrelax_even = createKernel("overrelax_even") << sources << "operations_heatbath.cl"
                                                    << "overrelax_even.cl";
    overrelax_odd = createKernel("overrelax_odd") << sources << "operations_heatbath.cl"
                                                  << "overrelax_odd.cl";
}
...
void hardware::code::Heatbath::run_overrelax(const hardware::buffers::SU3* gaugefield,
                                             const hardware::buffers::PRNGBuffer* prng) const
{
    cl_int clerr = CL_SUCCESS;
...
void hardware::code::Heatbath::get_work_sizes(const cl_kernel kernel, size_t* ls, size_t* gs, cl_uint* num_groups) const
{
    ...
    if (kernelname.compare("heatbath_even") == 0 || kernelname.compare("heatbath_odd") == 0 ||
        kernelname.compare("overrelax_even") == 0 || kernelname.compare("overrelax_odd") == 0) {
        if (get_device()->get_device_type() == CL_DEVICE_TYPE_GPU) {
            *gs = std::min(kernelParameters->getSpatialLatticeVolume() * kernelParameters->getNt() / 2,
                           hardware::buffers::get_prng_buffer_size(get_device(),
                                                                   kernelParameters->getUseSameRndNumbers()));
        } else {
            *gs = std::min(get_device()->get_num_compute_units(),
                           hardware::buffers::get_prng_buffer_size(get_device(),
                                                                   kernelParameters->getUseSameRndNumbers()));
        }
...
uint64_t hardware::code::Heatbath::get_flop_size(const std::string& in) const
{
    const size_t VOL4D = kernelParameters->getLatticeVolume();
    // this is the same as in the function above
    ///@NOTE: I do not distinguish between su3 and 3x3 matrices. This is a difference if one use e.g. REC12, but here
    /// one wants to have the "netto" flops for comparability.
    if ((in == "heatbath_even") || (in == "heatbath_odd")) {
        // this kernel calculates 1 staple (= 4*ND-1 su3_su3 + 2_ND-1 su3_add) plus NC*(2*su3_su3 80 flops for the su2
        // update)
        return VOL4D / 2 *
               (4 * (NDIM - 1) * getFlopSu3MatrixTimesSu3Matrix() + 2 * (NDIM - 1) * 18 +
                NC * (2 * getFlopSu3MatrixTimesSu3Matrix() + 80));
    }
    if ((in == "overrelax_even") || (in == "overrelax_odd")) {
        // this kernel calculates 1 staple (= 4*ND-1 su3_su3 + 2_ND-1 su3_add) plus NC*(2*su3_su3 58 flops for the su2
        // update)
        return VOL4D / 2 *
               (4 * (NDIM - 1) * getFlopSu3MatrixTimesSu3Matrix() + 2 * (NDIM - 1) * 18 +
                NC * (2 * getFlopSu3MatrixTimesSu3Matrix() + 58));
    }
    return 0;
}
...
hardware::code::Heatbath::Heatbath(const hardware::code::OpenClKernelParametersInterface& kernelParameters,
