...
struct TwistedMassMassParameters : public WilsonMassParameters {
    TwistedMassMassParameters(const double kappaIn, const double muIn) : WilsonMassParameters(kappaIn), mu(muIn) {}
...
struct FermionTestParameters : public SpinorTestParameters, GaugefieldTestParameters {
    FermionTestParameters(const LatticeExtents lE, const SpinorFillType spinorFillTypeIn,
                          const GaugefieldFillType gaugefieldFillTypesIn)
        : TestParameters(lE)
...
struct StaggeredFermionsTestParameters : public SpinorStaggeredTestParameters {
    StaggeredFermionsTestParameters(const LatticeExtents lE, const SpinorFillType spinorFillTypeIn,
                                    const GaugefieldFillType gaugefieldFillTypeIn,
                                    const WilsonMassParameters massParametersIn,
                                    const ThetaParameters thetaParametersIn = {0., 0.})
        : TestParameters(lE)
        , SpinorStaggeredTestParameters(lE)
        , gaugefieldFillType(gaugefieldFillTypeIn)
        , spinorFillType(spinorFillTypeIn)
        , massParameters(massParametersIn)
        , thetaParameters(thetaParametersIn){};
    StaggeredFermionsTestParameters(const LatticeExtents lE, const SpinorFillType spinorFillTypeIn,
                                    const GaugefieldFillType gaugefieldFillTypeIn,
                                    const ThetaParameters thetaParametersIn = {0., 0.})
        : TestParameters(lE)
...
struct EvenOddStaggeredFermionsTestParameters : public StaggeredFermionsTestParameters {
    EvenOddStaggeredFermionsTestParameters(const LatticeExtents lE, const SpinorFillType spinorFillTypeIn,
                                           const GaugefieldFillType gaugefieldFillTypeIn, const bool evenOrOddIn,
                                           const ThetaParameters thetaParametersIn              = {0., 0.},
                                           const ImaginaryChemicalPotential chemicalPotentialIn = 0.)
        : TestParameters(lE)
        , StaggeredFermionsTestParameters(lE, spinorFillTypeIn, gaugefieldFillTypeIn, thetaParametersIn)
        , evenOrOdd(evenOrOddIn)
        , chemicalPotential(chemicalPotentialIn)
    {
    }
...
struct FermionMatrixTestParameters : public FermionTestParameters {
    FermionMatrixTestParameters(const LatticeExtents latticeExtentsIn, const SpinorFillType spinorFillTypeIn,
                                const GaugefieldFillType gaugefieldFillTypeIn, const MassParameters massParametersIn)
        : TestParameters(latticeExtentsIn)
        , FermionTestParameters(latticeExtentsIn, spinorFillTypeIn, gaugefieldFillTypeIn)
        , massParameters(massParametersIn){};
    FermionMatrixTestParameters(const LatticeExtents latticeExtentsIn, const SpinorFillType spinorFillTypeIn,
                                const MassParameters massParametersIn)
        : TestParameters(latticeExtentsIn)
        , FermionTestParameters(latticeExtentsIn, spinorFillTypeIn, GaugefieldFillType::cold)
        , massParameters(massParametersIn){};
...
struct FermionmatrixTester : public KernelTester {
    FermionmatrixTester(std::string kernelName, const ParameterCollection parameterCollection,
                        const FermionTestParameters testParameters, const ReferenceValues rV)
        : KernelTester(kernelName, parameterCollection.hardwareParameters, parameterCollection.kernelParameters,
                       testParameters, rV)
    {
        GaugefieldCreator gf(testParameters.latticeExtents);
        gaugefieldBuffer         = new hardware::buffers::SU3(calculateGaugefieldSize(
...
struct StaggeredFermionmatrixTester : public KernelTester {
    StaggeredFermionmatrixTester(std::string kernelName, const ParameterCollection parameterCollection,
                                 const StaggeredFermionsTestParameters testParameters, const ReferenceValues rV)
        : KernelTester(kernelName, parameterCollection.hardwareParameters, parameterCollection.kernelParameters,
                       testParameters, rV)
    {
        GaugefieldCreator gf(testParameters.latticeExtents);
...
struct NonEvenOddFermionmatrixTester
    : public FermionmatrixTester<hardware::buffers::Plain<spinor>, NonEvenOddSpinorfieldCreator> {
    NonEvenOddFermionmatrixTester(std::string kernelName, const ParameterCollection parameterCollection,
                                  const FermionTestParameters testParameters, const ReferenceValues rV)
        : FermionmatrixTester<hardware::buffers::Plain<spinor>, NonEvenOddSpinorfieldCreator>(kernelName,
                                                                                              parameterCollection,
                                                                                              testParameters, rV)
    {
    }
...
struct EvenOddFermionmatrixTester : public FermionmatrixTester<hardware::buffers::Spinor, EvenOddSpinorfieldCreator> {
    EvenOddFermionmatrixTester(std::string kernelName, const ParameterCollection parameterCollection,
                               const FermionTestParameters testParameters, const ReferenceValues rV)
        : FermionmatrixTester<hardware::buffers::Spinor, EvenOddSpinorfieldCreator>(kernelName, parameterCollection,
                                                                                    testParameters, rV)
    {
    }
};
...
struct FermionmatrixTesterWithSumAsKernelResult : public TesterType {
    FermionmatrixTesterWithSumAsKernelResult(const std::string kernelName,
                                             const ParameterCollection parameterCollection,
                                             const FermionTestParameters testParameters, const ReferenceValues rV)
...
struct StaggeredFermionmatrixTesterWithSumAsKernelResult : public TesterType {
    StaggeredFermionmatrixTesterWithSumAsKernelResult(const std::string kernelName,
                                                      const ParameterCollection parameterCollection,
                                                      const StaggeredFermionsTestParameters testParameters,
