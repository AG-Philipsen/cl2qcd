...
struct ExtendTestCode : public TestCode {
    ExtendTestCode(const hardware::code::OpenClKernelParametersInterface& kP, hardware::Device* device)
        : TestCode(kP, device)
    {
        testKernel = createKernel("extendKernel") << get_device()->getGaugefieldCode()->get_sources()
                                                  << "../hardware/code/miscellaneousTests/su2su3extend.cl";
    }

    void runExtendKernel(const hardware::buffers::Plain<Matrixsu3>* out, const hardware::buffers::Plain<Matrixsu2>* in,
                         const hardware::buffers::Plain<cl_int>* d_rand, cl_ulong elems)
    {
        cl_int err;
