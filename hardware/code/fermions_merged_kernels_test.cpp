...
calculateReferenceValues_saxpyAndGamma5EvenOdd(const int latticeVolume, const hmc_complex coefficient)
{
...
const ReferenceValues
calculateReferenceValues_mTmSitediagonalAndGamma5EvenOdd(const int latticeVolume,
...
const ReferenceValues
calculateReferenceValues_mTmSitediagonalMinusAndGamma5EvenOdd(const int latticeVolume,
                                                              const TwistedMassMassParameters massParameters)
{
    if (massParameters.kappa == 0. and massParameters.mu == 0.) {
        return ReferenceValues{latticeVolume * (-144.)};
...
const ReferenceValues
calculateReferenceValues_dslashEvenOddAndMTmInverseSitediagonalMinus(const int latticeVolume,
                                                                     const GaugefieldFillType gF,
                                                                     const TwistedMassMassParameters massParameters)
{
    ...
    } else if (gF == GaugefieldFillType::nonTrivial and massParameters.kappa == 3 * nonTrivialParameter and
               massParameters.mu == -7 * nonTrivialParameter) {
...
const ReferenceValues
calculateReferenceValues_dslashEvenOddAndMTmInverseSitediagonal(const int latticeVolume, const GaugefieldFillType gF,
                                                                const TwistedMassMassParameters massParameters)
{
    if (gF == GaugefieldFillType::cold) {
        if (massParameters.kappa == 1. and massParameters.mu == 0.) {
    ...
    } else if (gF == GaugefieldFillType::nonTrivial and massParameters.kappa == -2 * nonTrivialParameter and
               massParameters.mu == -8 * nonTrivialParameter) {
...
struct SaxpyAndGamma5EvenOddTestParameters : public FermionTestParameters {
    SaxpyAndGamma5EvenOddTestParameters(const LatticeExtents latticeExtentsIn, const SpinorFillTypes spinorFillTypesIn,
                                        const hmc_complex coefficientIn)
        : TestParameters(latticeExtentsIn)
        , FermionTestParameters(latticeExtentsIn, spinorFillTypesIn)
...
struct SaxpyAndGamma5EvenOddTester : public FermionmatrixTesterWithSumAsKernelResult<EvenOddFermionmatrixTester> {
    SaxpyAndGamma5EvenOddTester(const ParameterCollection& pc, const SaxpyAndGamma5EvenOddTestParameters& tP)
        : FermionmatrixTesterWithSumAsKernelResult<
              EvenOddFermionmatrixTester>("saxpy_AND_gamma5", pc, tP,
                                          calculateReferenceValues_saxpyAndGamma5EvenOdd(calculateEvenOddSpinorfieldSize(
...
struct MTmSitediagonalMinusAndGamma5EvenOddTester
    : public FermionmatrixTesterWithSumAsKernelResult<EvenOddFermionmatrixTester> {
    MTmSitediagonalMinusAndGamma5EvenOddTester(const ParameterCollection& pc, const TwistedMassTestParameters& tP)
        : FermionmatrixTesterWithSumAsKernelResult<
              EvenOddFermionmatrixTester>("m_tm_sitediagonal_minus_AND_gamma5_eo", pc, tP,
                                          calculateReferenceValues_mTmSitediagonalMinusAndGamma5EvenOdd(calculateEvenOddSpinorfieldSize(
                                                                                                            tP.SpinorTestParameters::
                                                                                                                latticeExtents),
                                                                                                        tP.massParameters))
    {
...
struct MTmSitediagonalAndGamma5EvenOddTester
    : public FermionmatrixTesterWithSumAsKernelResult<EvenOddFermionmatrixTester> {
    MTmSitediagonalAndGamma5EvenOddTester(const ParameterCollection& pc, const TwistedMassTestParameters& tP)
        : FermionmatrixTesterWithSumAsKernelResult<
              EvenOddFermionmatrixTester>("m_tm_sitediagonal_AND_gamma5_eo", pc, tP,
                                          calculateReferenceValues_mTmSitediagonalAndGamma5EvenOdd(calculateEvenOddSpinorfieldSize(
                                                                                                       tP.SpinorTestParameters::
                                                                                                           latticeExtents),
                                                                                                   tP.massParameters))
    {
...
struct DslashEvenOddAndMTmInverseSitediagonalMinusTester
    : public FermionmatrixTesterWithSumAsKernelResult<EvenOddFermionmatrixTester> {
    DslashEvenOddAndMTmInverseSitediagonalMinusTester(const ParameterCollection& pc,
                                                      const TwistedMassTestParameters& tP, const bool evenOrOdd)
        : FermionmatrixTesterWithSumAsKernelResult<
              EvenOddFermionmatrixTester>("dslash_AND_m_tm_inverse_sitediagonal_minus", pc, tP,
                                          calculateReferenceValues_dslashEvenOddAndMTmInverseSitediagonalMinus(calculateEvenOddSpinorfieldSize(
                                                                                                                   tP.SpinorTestParameters::
                                                                                                                       latticeExtents),
                                                                                                               tP.GaugefieldTestParameters::
                                                                                                                   fillType,
                                                                                                               tP.massParameters))
    {
        code->dslash_AND_M_tm_inverse_sitediagonal_minus_eo_device(in, out, gaugefieldBuffer, evenOrOdd,
                                                                   tP.massParameters.kappa,
                                                                   tP.massParameters.getMubar());
    }
};

struct DslashEvenOddAndMTmInverseSitediagonalTester
    : public FermionmatrixTesterWithSumAsKernelResult<EvenOddFermionmatrixTester> {
    DslashEvenOddAndMTmInverseSitediagonalTester(const ParameterCollection& pc, const TwistedMassTestParameters& tP,
                                                 const bool evenOrOdd)
        : FermionmatrixTesterWithSumAsKernelResult<
              EvenOddFermionmatrixTester>("dslash_AND_m_tm_inverse_sitediagonal", pc, tP,
                                          calculateReferenceValues_dslashEvenOddAndMTmInverseSitediagonal(calculateEvenOddSpinorfieldSize(
                                                                                                              tP.SpinorTestParameters::
                                                                                                                  latticeExtents),
                                                                                                          tP.GaugefieldTestParameters::
                                                                                                              fillType,
                                                                                                          tP.massParameters))
    {
        code->dslash_AND_M_tm_inverse_sitediagonal_eo_device(in, out, gaugefieldBuffer, evenOrOdd,
                                                             tP.massParameters.kappa, tP.massParameters.getMubar());
    }
};
...
void testSaxpyAndGamma5EvenOdd(const LatticeExtents lE, const SpinorFillTypes sF, const hmc_complex coefficient)
{
    ...
    hardware::code::OpenClKernelParametersMockupForMergedFermionKernels kernelParameters(parametersForThisTest.ns,
...
void runTest(const LatticeExtents lE, const SpinorFillType sF, const TwistedMassMassParameters mP)
{
    ...
    hardware::code::OpenClKernelParametersMockupForTwistedMass kernelParameters(parametersForThisTest.ns,
                                                                                parametersForThisTest.nt, true, true);
    ParameterCollection parameterCollection{hardwareParameters, kernelParameters};
    TesterClass(parameterCollection, parametersForThisTest);
}

template<class TesterClass>
void runTest(const LatticeExtents lE, const SpinorFillType sF, const GaugefieldFillType gF,
             const TwistedMassMassParameters mP, const bool evenOrOdd)
{
...
void testMTmSitediagonalMinusAndGamma5EvenOdd(const LatticeExtents lE, const SpinorFillType sF,
                                              const TwistedMassMassParameters mP)
{
    runTest<MTmSitediagonalMinusAndGamma5EvenOddTester>(lE, sF, mP);
}

void testMTmSitediagonalAndGamma5EvenOdd(const LatticeExtents lE, const SpinorFillType sF,
                                         const TwistedMassMassParameters mP)
{
    runTest<MTmSitediagonalAndGamma5EvenOddTester>(lE, sF, mP);
}

void testDslashEvenOddAndMTmInverseSitediagonalMinus(const LatticeExtents lE, const SpinorFillType sF,
                                                     const GaugefieldFillType gF, const TwistedMassMassParameters mP,
                                                     const bool evenOrOdd)
{
    runTest<DslashEvenOddAndMTmInverseSitediagonalMinusTester>(lE, sF, gF, mP, evenOrOdd);
}

void testDslashEvenOddAndMTmInverseSitediagonal(const LatticeExtents lE, const SpinorFillType sF,
                                                const GaugefieldFillType gF, const TwistedMassMassParameters mP,
                                                const bool evenOrOdd)
{
...
BOOST_AUTO_TEST_SUITE(DSLASH_AND_M_TM_INVERSE_SITEDIAGONAL_EO)
    ...
    {
        testDslashEvenOddAndMTmInverseSitediagonal(LatticeExtents{ns8, nt8}, SpinorFillType::ascendingComplex,
                                                   GaugefieldFillType::cold, TwistedMassMassParameters{1, 0.}, true);
    }

    BOOST_AUTO_TEST_CASE(DSLASH_AND_M_TM_INVERSE_SITEDIAGONAL_EO_2)
    {
        testDslashEvenOddAndMTmInverseSitediagonal(LatticeExtents{ns8, nt8}, SpinorFillType::ascendingComplex,
                                                   GaugefieldFillType::cold, TwistedMassMassParameters{1, 0.}, false);
    }

    BOOST_AUTO_TEST_CASE(DSLASH_AND_M_TM_INVERSE_SITEDIAGONAL_EO_3)
    {
        testDslashEvenOddAndMTmInverseSitediagonal(LatticeExtents{ns8, nt8}, SpinorFillType::ascendingComplex,
                                                   GaugefieldFillType::cold,
                                                   TwistedMassMassParameters{-2 * nonTrivialParameter,
                                                                             -8 * nonTrivialParameter},
                                                   true);
    }
...
BOOST_AUTO_TEST_SUITE(DSLASH_AND_M_TM_INVERSE_SITEDIAGONAL_MINUS_EO)
    ...
    {
        testDslashEvenOddAndMTmInverseSitediagonalMinus(LatticeExtents{ns8, nt8}, SpinorFillType::ascendingComplex,
                                                        GaugefieldFillType::cold, TwistedMassMassParameters{1, 0.},
                                                        true);
    }
    ...
    BOOST_AUTO_TEST_CASE(DSLASH_AND_M_TM_INVERSE_SITEDIAGONAL_MINUS_EO_3)
    {
        testDslashEvenOddAndMTmInverseSitediagonalMinus(LatticeExtents{ns8, nt8}, SpinorFillType::ascendingComplex,
                                                        GaugefieldFillType::cold,
                                                        TwistedMassMassParameters{3. * nonTrivialParameter,
                                                                                  -7 * nonTrivialParameter},
                                                        true);
    }
...
BOOST_AUTO_TEST_SUITE(M_TM_SITEDIAGONAL_AND_GAMMA5_EO)
    ...
    {
        testMTmSitediagonalAndGamma5EvenOdd(LatticeExtents{ns8, nt8}, SpinorFillType::ascendingComplex,
                                            TwistedMassMassParameters{0., 0.});
    }

    BOOST_AUTO_TEST_CASE(M_TM_SITEDIAGONAL_AND_GAMMA5_EO_2)
    {
        testMTmSitediagonalAndGamma5EvenOdd(LatticeExtents{ns8, nt8}, SpinorFillType::ascendingComplex,
                                            TwistedMassMassParameters{-4. * nonTrivialParameter,
                                                                      11. * nonTrivialParameter});
    }
...
BOOST_AUTO_TEST_SUITE(M_TM_SITEDIAGONAL_MINUS_AND_GAMMA5_EO)
    ...
    {
        testMTmSitediagonalMinusAndGamma5EvenOdd(LatticeExtents{ns4, nt8}, SpinorFillType::ascendingComplex,
                                                 TwistedMassMassParameters{0., 0.});
    }
...
BOOST_AUTO_TEST_SUITE(SAXPY_AND_GAMMA5_EO)
    ...
    {
        testSaxpyAndGamma5EvenOdd(LatticeExtents{ns4, nt8},
    ...
    {
        testSaxpyAndGamma5EvenOdd(LatticeExtents{ns4, nt12},
                                  SpinorFillTypes{SpinorFillType::ascendingComplex, SpinorFillType::ascendingComplex},
                                  hmc_complex{-nonTrivialParameter, 2. * nonTrivialParameter});
    }
