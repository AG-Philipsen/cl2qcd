...
void hardware::code::Fermions::fill_kernels()
{
    ...
    if (kernelParameters->getFermact() == common::action::wilson) {
        M_wilson = createKernel("M_wilson") << sources << "fermionmatrix.cl"
                                            << "fermionmatrix_m.cl";
    } else if (kernelParameters->getFermact() == common::action::twistedmass) {
        M_tm_plus = createKernel("M_tm_plus") << sources << "fermionmatrix.cl"
                                              << "fermionmatrix_m_tm_plus.cl";
        M_tm_minus = createKernel("M_tm_minus") << sources << "fermionmatrix.cl"
                                                << "fermionmatrix_m_tm_minus.cl";
    } else if (kernelParameters->getFermact() == common::action::clover) {
        throw Print_Error_Message("no kernels for CLOVER-discretization implemented yet, aborting... ", __FILE__,
                                  __LINE__);
    } else {
        throw Print_Error_Message("there was a problem with which fermion-discretization to use, aborting... ",
                                  __FILE__, __LINE__);
    }

    gamma5 = createKernel("gamma5") << sources << "fermionmatrix.cl"
                                    << "fermionmatrix_gamma5.cl";
    ...
    if (kernelParameters->getUseEo() == true) {
        if (kernelParameters->getFermact() == common::action::twistedmass) {
            M_tm_sitediagonal = createKernel("M_tm_sitediagonal") << sources << "fermionmatrix.cl"
                                                                  << "fermionmatrix_eo.cl"
                                                                  << "fermionmatrix_eo_m.cl";
            M_tm_inverse_sitediagonal = createKernel("M_tm_inverse_sitediagonal") << sources << "fermionmatrix.cl"
                                                                                  << "fermionmatrix_eo.cl"
                                                                                  << "fermionmatrix_eo_m.cl";
            M_tm_sitediagonal_minus = createKernel("M_tm_sitediagonal_minus") << sources << "fermionmatrix.cl"
                                                                              << "fermionmatrix_eo.cl"
                                                                              << "fermionmatrix_eo_m.cl";
            M_tm_inverse_sitediagonal_minus = createKernel("M_tm_inverse_sitediagonal_minus")
                                              << sources << "fermionmatrix.cl"
                                              << "fermionmatrix_eo.cl"
                                              << "fermionmatrix_eo_m.cl";
        }
        dslash_eo = createKernel("dslash_eo") << sources << "fermionmatrix.cl"
                                              << "fermionmatrix_eo.cl"
                                              << "fermionmatrix_eo_dslash.cl";
        _dslash_eo_boundary = createKernel("dslash_eo_boundary") << sources << "fermionmatrix.cl"
                                                                 << "fermionmatrix_eo.cl"
                                                                 << "fermionmatrix_eo_dslash.cl";
        _dslash_eo_inner = createKernel("dslash_eo_inner") << sources << "fermionmatrix.cl"
                                                           << "fermionmatrix_eo.cl"
                                                           << "fermionmatrix_eo_dslash.cl";
        gamma5_eo = createKernel("gamma5_eo") << sources << "fermionmatrix.cl"
                                              << "fermionmatrix_eo_gamma5.cl";
        // merged kernels
        if (kernelParameters->getUseMergeKernelsFermion() == true) {
            dslash_AND_M_tm_inverse_sitediagonal_eo = createKernel("dslash_AND_M_tm_inverse_sitediagonal_eo")
                                                      << sources << "fermionmatrix.cl"
                                                      << "fermionmatrix_eo.cl"
                                                      << "fermionmatrix_eo_dslash_AND_M_tm_inverse_sitediagonal.cl";
            dslash_AND_M_tm_inverse_sitediagonal_minus_eo =
                createKernel("dslash_AND_M_tm_inverse_sitediagonal_minus_eo")
                << sources << "fermionmatrix.cl"
                << "fermionmatrix_eo.cl"
                << "fermionmatrix_eo_dslash_AND_M_tm_inverse_sitediagonal_minus.cl";
            M_tm_sitediagonal_AND_gamma5_eo = createKernel("M_tm_sitediagonal_AND_gamma5_eo")
                                              << sources << "fermionmatrix.cl"
                                              << "fermionmatrix_eo.cl"
                                              << "fermionmatrix_eo_m_merged.cl";
            M_tm_sitediagonal_minus_AND_gamma5_eo = createKernel("M_tm_sitediagonal_minus_AND_gamma5_eo")
                                                    << sources << "fermionmatrix.cl"
                                                    << "fermionmatrix_eo.cl"
                                                    << "fermionmatrix_eo_m_merged.cl";
            saxpy_AND_gamma5_eo = createKernel("saxpy_AND_gamma5_eo") << sources << "fermionmatrix.cl"
                                                                      << "fermionmatrix_eo.cl"
                                                                      << "fermionmatrix_saxpy_AND_gamma5_eo.cl";
        }
    }
}
...
void hardware::code::Fermions::M_wilson_device(const hardware::buffers::Plain<spinor>* in,
                                               const hardware::buffers::Plain<spinor>* out,
                                               const hardware::buffers::SU3* gf, hmc_float kappa) const
{
    // get kappa
    hmc_float kappa_tmp;
    if (kappa == ARG_DEF)
...
void hardware::code::Fermions::M_tm_plus_device(const hardware::buffers::Plain<spinor>* in,
                                                const hardware::buffers::Plain<spinor>* out,
                                                const hardware::buffers::SU3* gf, hmc_float kappa,
                                                hmc_float mubar) const
{
    // get kappa
    hmc_float kappa_tmp;
    if (kappa == ARG_DEF)
    ...
    hmc_float mubar_tmp;
    if (mubar == ARG_DEF)
        mubar_tmp = kernelParameters->getMuBar();
    else
        mubar_tmp = mubar;
...
void hardware::code::Fermions::M_tm_minus_device(const hardware::buffers::Plain<spinor>* in,
                                                 const hardware::buffers::Plain<spinor>* out,
                                                 const hardware::buffers::SU3* gf, hmc_float kappa,
                                                 hmc_float mubar) const
{
...
void hardware::code::Fermions::saxpy_AND_gamma5_eo_device(const hardware::buffers::Spinor* x,
                                                          const hardware::buffers::Spinor* y, const hmc_complex alpha,
                                                          const hardware::buffers::Spinor* out) const
{
    // query work-sizes for kernel
    size_t ls2, gs2;
    cl_uint num_groups;
...
void hardware::code::Fermions::dslash_eo_device(const hardware::buffers::Spinor* in,
                                                const hardware::buffers::Spinor* out, const hardware::buffers::SU3* gf,
                                                int evenodd, hmc_float kappa) const
{
    // get kappa
    hmc_float kappa_tmp;
    if (kappa == ARG_DEF)
...
void hardware::code::Fermions::dslash_eo_boundary(const hardware::buffers::Spinor* in,
                                                  const hardware::buffers::Spinor* out,
                                                  const hardware::buffers::SU3* gf, int evenodd, hmc_float kappa) const
{
    // get kappa
    hmc_float kappa_tmp;
    if (kappa == ARG_DEF)
...
void hardware::code::Fermions::dslash_eo_inner(const hardware::buffers::Spinor* in,
                                               const hardware::buffers::Spinor* out, const hardware::buffers::SU3* gf,
                                               int evenodd, hmc_float kappa) const
{
    // get kappa
...
void hardware::code::Fermions::dslash_AND_M_tm_inverse_sitediagonal_eo_device(const hardware::buffers::Spinor* in,
                                                                              const hardware::buffers::Spinor* out,
                                                                              const hardware::buffers::SU3* gf,
                                                                              int evenodd, hmc_float kappa,
                                                                              hmc_float mubar) const
{
    cl_int eo = evenodd;
    // query work-sizes for kernel
...
void hardware::code::Fermions::dslash_AND_M_tm_inverse_sitediagonal_minus_eo_device(
    const hardware::buffers::Spinor* in, const hardware::buffers::Spinor* out, const hardware::buffers::SU3* gf,
    int evenodd, hmc_float kappa, hmc_float mubar) const
{
    cl_int eo = evenodd;
    // query work-sizes for kernel
    size_t ls2, gs2;
...
void hardware::code::Fermions::M_tm_inverse_sitediagonal_device(const hardware::buffers::Spinor* in,
                                                                const hardware::buffers::Spinor* out,
                                                                hmc_float mubar) const
{
    // get mu
...
void hardware::code::Fermions::M_tm_sitediagonal_device(const hardware::buffers::Spinor* in,
                                                        const hardware::buffers::Spinor* out, hmc_float mubar) const
{
    // get mu
    hmc_float mubar_tmp;
    if (mubar == ARG_DEF)
...
void hardware::code::Fermions::M_tm_sitediagonal_AND_gamma5_eo_device(const hardware::buffers::Spinor* in,
                                                                      const hardware::buffers::Spinor* out,
                                                                      hmc_float mubar) const
{
    // get mu
...
void hardware::code::Fermions::M_tm_sitediagonal_minus_AND_gamma5_eo_device(const hardware::buffers::Spinor* in,
                                                                            const hardware::buffers::Spinor* out,
                                                                            hmc_float mubar) const
{
    // get mu
...
void hardware::code::Fermions::M_tm_inverse_sitediagonal_minus_device(const hardware::buffers::Spinor* in,
                                                                      const hardware::buffers::Spinor* out,
                                                                      hmc_float mubar) const
{
    // get mu
...
void hardware::code::Fermions::M_tm_sitediagonal_minus_device(const hardware::buffers::Spinor* in,
                                                              const hardware::buffers::Spinor* out,
                                                              hmc_float mubar) const
{
    // get mu
...
{
    /** @NOTE: this is the "original" dslash without any simplifications, counting everything "full". this is a much too
      hight number!!
      ...
      */
    return NDIM * 2 *
           (hardware::code::getFlopSu3MatrixTimesSu3Vec() * 2 + 6 * NC * 2 +
            2 * NC * hardware::code::getFlopComplexMult());
    /// I do not know for sure, but if one leaves out the 2 su3vec_complex operations in each directions one has almost
    /// 1320 flop per site. This would correspond to have the kappa in the diagonal matrix still.
}
...
hardware::code::Fermions::Fermions(const hardware::code::OpenClKernelParametersInterface& kernelParameters,
