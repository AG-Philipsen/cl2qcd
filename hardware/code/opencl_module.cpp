 ...
 */
static void print_profiling(const std::string& filename, const std::string& kernelName,
...
static std::string collect_basic_options(const hardware::Device* device,
                                         const hardware::code::OpenClKernelParametersInterface& kernelParameters)
{
    ...
    switch (kernelParameters.getFermact()) {
        ...
        case common::action::rooted_stagg:
            options << " -D _RHMC_";
            options << " -D RA_MAX_ORDER="
                    << std::max(kernelParameters.getMetroApproxOrd(), kernelParameters.getMdApproxOrd());
            break;
    ...
    // BC: on the corners in each direction: exp(i theta) -> on each site exp(i theta*PI /LATEXTENSION) = cos(tmp2) +
    // isin(tmp2)
...
hardware::code::Opencl_Module::Opencl_Module(const hardware::code::OpenClKernelParametersInterface& kernelParameters,
...
void hardware::code::Opencl_Module::get_work_sizes(const cl_kernel kernel, size_t* ls, size_t* gs,
                            ...
                            uint64_t sites)
{
    ...
    if (data.get_num_values()) {
        ...
        bandwidth = (hmc_float)read_write_size / (hmc_float)data.get_total_time() * (hmc_float)data.get_num_values() *
                    1e-3;
    ...
    out.width(32);
    out.precision(15);
    out << kernelName << "\t" << data.get_total_time() << "\t" << data.get_num_values() << "\t" << avg_time << "\t"
        << avg_time_site << "\t" << bandwidth << "\t" << flops << "\t" << (float)read_write_size / mega << "\t"
        << flop_size << std::endl;
    out.close();
}
...
static void print_profile_header(const std::string& filename, int number)
{
    ...
    out.precision(15);
    out << "#device " << number
        << "\tTime [mus]\tCalls\tAvg Time [mus]\tAvg Time/Site [mus]\tBW [GB/s]\tFLOPS [GFLOP/s]\tRe/Wr [MB]\tFLOP"
        << std::endl;
}
...
void hardware::code::Opencl_Module::print_profiling(const std::string& filename, const cl_kernel& kernel) const
{
    ...
    if (kernel) {
        ...
        ::print_profiling(filename, kernel_name, data, this->get_read_write_size(kernel_name),
                          this->get_flop_size(kernel_name), kernelParameters->getLatticeVolume());
    }
}
