...
SpinorStaggeredTester::SpinorStaggeredTester(std::string kN, const ParameterCollection pC,
...
su3vec*
NonEvenOddSpinorStaggeredfieldCreator::createSpinorfieldWithOnesAndZerosDependingOnSiteParity(const bool fillEvenSites)
{
    su3vec* in;
    in = new su3vec[numberOfElements];
...
su3vec* EvenOddSpinorStaggeredfieldCreator::createSpinorfieldEvenOddWithOnesAndZerosDependingOnSiteParity(
...
hmc_float count_sf(su3vec* sf_in, int size)
{
    hmc_float sum = 0.;
    for (int i = 0; i < size; i++) {
        sum += sf_in[i].e0.re + sf_in[i].e0.im + sf_in[i].e1.re + sf_in[i].e1.im + sf_in[i].e2.re + sf_in[i].e2.im;
    }
    return sum;
}
...
hmc_float squareNorm(su3vec* sf_in, int size)
{
    hmc_float sum = 0.;
    for (int i = 0; i < size; i++) {
        sum += sf_in[i].e0.re * sf_in[i].e0.re + sf_in[i].e0.im * sf_in[i].e0.im + sf_in[i].e1.re * sf_in[i].e1.re +
               sf_in[i].e1.im * sf_in[i].e1.im + sf_in[i].e2.re * sf_in[i].e2.re + sf_in[i].e2.im * sf_in[i].e2.im;
    }
    return sum;
}
...
hmc_float calc_var_sf(su3vec* sf_in, int size, hmc_float sum)
{
    ...
    for (int k = 0; k < size; k++) {
        var += calc_var(sf_in[k].e0.re, sum) + calc_var(sf_in[k].e0.im, sum) + calc_var(sf_in[k].e1.re, sum) +
               calc_var(sf_in[k].e1.im, sum) + calc_var(sf_in[k].e2.re, sum) + calc_var(sf_in[k].e2.im, sum);
    }
    return var;
}
