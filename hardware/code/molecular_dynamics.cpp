...
{
    basic_molecular_dynamics_code = get_basic_sources() << "operations_geometry.cl"
    ...
    if (kernelParameters->getUseEo() == true) {
        fermion_force_eo_0 = createKernel("fermion_force_eo_0")
                             << basic_molecular_dynamics_code << "operations_spinorfield_eo.cl"
                             << "fermionmatrix.cl"
                             << "force_fermion_eo.cl";
        fermion_force_eo_1 = createKernel("fermion_force_eo_1")
                             << basic_molecular_dynamics_code << "operations_spinorfield_eo.cl"
                             << "fermionmatrix.cl"
                             << "force_fermion_eo.cl";
        fermion_force_eo_2 = createKernel("fermion_force_eo_2")
                             << basic_molecular_dynamics_code << "operations_spinorfield_eo.cl"
                             << "fermionmatrix.cl"
                             << "force_fermion_eo.cl";
        fermion_force_eo_3 = createKernel("fermion_force_eo_3")
                             << basic_molecular_dynamics_code << "operations_spinorfield_eo.cl"
                             << "fermionmatrix.cl"
                             << "force_fermion_eo.cl";
        fermion_stagg_partial_force_eo = createKernel("fermion_staggered_partial_force_eo")
                                         << basic_molecular_dynamics_code << "operations_staggered.cl"
                                         << "spinorfield_staggered_eo.cl"
                                         << "force_staggered_fermion_eo.cl";
    }
    fermion_force = createKernel("fermion_force") << basic_molecular_dynamics_code << "fermionmatrix.cl"
                                                  << "force_fermion.cl";
    md_update_gaugefield = createKernel("md_update_gaugefield")
                           << basic_molecular_dynamics_code << "md_update_gaugefield.cl";
    ...
    if (kernelParameters->getUseRectangles() == true) {
        ...
        if (gauge_force_tlsym_tmp) {
            gauge_force_tlsym   = 0;
            gauge_force_tlsym_1 = createKernel("gauge_force_tlsym_multipass1_tpe")
                                  << basic_molecular_dynamics_code << "force_gauge_tlsym.cl";
            gauge_force_tlsym_2 = createKernel("gauge_force_tlsym_multipass2_tpe")
                                  << basic_molecular_dynamics_code << "force_gauge_tlsym.cl";
            gauge_force_tlsym_3 = createKernel("gauge_force_tlsym_multipass3_tpe")
                                  << basic_molecular_dynamics_code << "force_gauge_tlsym.cl";
            gauge_force_tlsym_4 = createKernel("gauge_force_tlsym_multipass4_tpe")
                                  << basic_molecular_dynamics_code << "force_gauge_tlsym.cl";
            gauge_force_tlsym_5 = createKernel("gauge_force_tlsym_multipass5_tpe")
                                  << basic_molecular_dynamics_code << "force_gauge_tlsym.cl";
            gauge_force_tlsym_6 = createKernel("gauge_force_tlsym_multipass6_tpe")
                                  << basic_molecular_dynamics_code << "force_gauge_tlsym.cl";
        } else {
            gauge_force_tlsym = createKernel("gauge_force_tlsym")
                                << basic_molecular_dynamics_code << "force_gauge_tlsym.cl";
    ...
    if (kernelParameters->getUseSmearing() == true) {
        stout_smear_fermion_force = createKernel("stout_smear_fermion_force")
                                    << basic_molecular_dynamics_code << "force_fermion_stout_smear.cl";
    }
}
...
void hardware::code::Molecular_Dynamics::get_work_sizes(const cl_kernel kernel, size_t* ls, size_t* gs,
...
size_t hardware::code::Molecular_Dynamics::get_read_write_size(const std::string& in) const
{
    ...
    if (in == "gauge_force_tlsym") {
        ...
        // the rect staple is the same as the normal staple, but with 3 add. contributions and 2 add. matrices in each
        // contribution, so instead of 2 * 3 = 6, one reads 6 * 5 matrices in each direction
...
uint64_t hardware::code::Molecular_Dynamics::get_flop_size(const std::string& in) const
{
    ...
    if (in == "gauge_force") {
        // this kernel calculates 1 staple (= 4*ND-1 su3_su3 + 2*ND-1 su3_add), 1 su3*su3, 1 tr_lambda_u (19 flops) plus
        // 8 add and 8 mult per ae
        return (4 * (NDIM - 1) * getFlopSu3MatrixTimesSu3Matrix() + 2 * (NDIM - 1) * 18 +
                1 * getFlopSu3MatrixTimesSu3Matrix() + 19 + A * (1 + 1)) *
               G;
    }
    if (in == "gauge_force_tlsym") {
        // this kernel calculates 1 rect-staple (= 24*ND-1 su3_su3 + 6*ND-1 su3_add), 1 su3*su3, 1 tr_lambda_u (19
        // flops) plus 8 add and 8 mult per ae 24 = 6 contr. per dir, 4 mat_mat per contr.
        return (24 * (NDIM - 1) * getFlopSu3MatrixTimesSu3Matrix() + 6 * (NDIM - 1) * 18 +
                1 * getFlopSu3MatrixTimesSu3Matrix() + 19 + A * (1 + 1)) *
               G;
    }
    if (in == "fermion_force") {
        // this kernel performs NDIM * ( 4 * su3vec_acc (6 flops) + tr(v*u) (126 flops) + tr_lambda_u(19 flops) +
        // update_ae(8*2 flops) + su3*su3 + su3*complex (flop_complex_mult * R ) ) per site NOTE: the kernel now runs
        // over all ae instead of all sites, but this must be equivalent!
        return Seo * NDIM * (4 * 6 + 126 + 19 + 8 * 2 + getFlopSu3MatrixTimesSu3Matrix() + getFlopComplexMult() * R);
    }
    if (in == "fermion_force_eo") {
        // this kernel performs NDIM * ( 4 * su3vec_acc (6 flops) + tr(v*u) (126 flops) + tr_lambda_u(19 flops) +
        // update_ae(8*2 flops) + su3*su3 + su3*complex (flop_complex_mult * R ) ) per site_eo
        return Seo * NDIM * (4 * 6 + 126 + 19 + 8 * 2 + getFlopSu3MatrixTimesSu3Matrix() + getFlopComplexMult() * R);
    }
    if (in == "fermion_staggered_partial_force_eo") {
        ...
        return Seo * NDIM *
               (6 + getFlopSu3VecDirectSu3Vec() + getFlopSu3MatrixTimesSu3Matrix() + 18 + R * 18 +
                getFlopComplexMult() + 9 + 16);
    }
    if (in == "stout_smear_fermion_force") {
        return module_metric_not_implemented<uint64_t>();
    }
...
void hardware::code::Molecular_Dynamics::md_update_gaugefield_device(const hardware::buffers::Gaugemomentum* gm_in,
                                                                     const hardware::buffers::SU3* gf_out,
                                                                     hmc_float eps) const
{
...
void hardware::code::Molecular_Dynamics::gauge_force_device(const hardware::buffers::SU3* gf,
...
void hardware::code::Molecular_Dynamics::gauge_force_tlsym_device(const hardware::buffers::SU3* gf,
                                                                  const hardware::buffers::Gaugemomentum* out) const
{
    if (gauge_force_tlsym_tmp) {
        // run multipass
...
void hardware::code::Molecular_Dynamics::fermion_force_device(const hardware::buffers::Plain<spinor>* Y,
                                                              const hardware::buffers::Plain<spinor>* X,
                                                              const hardware::buffers::SU3* gf,
                                                              const hardware::buffers::Gaugemomentum* out,
                                                              hmc_float kappa) const
{
...
void hardware::code::Molecular_Dynamics::fermion_force_eo_device(const hardware::buffers::Spinor* Y,
...
void hardware::code::Molecular_Dynamics::fermion_staggered_partial_force_device(
    const hardware::buffers::SU3* gf, const hardware::buffers::SU3vec* A, const hardware::buffers::SU3vec* B,
    const hardware::buffers::Gaugemomentum* out, int evenodd) const
{
    using namespace hardware::buffers;
...
void hardware::code::Molecular_Dynamics::stout_smeared_fermion_force_device(
    std::vector<const hardware::buffers::SU3*>&) const
{
...
hardware::code::Molecular_Dynamics::Molecular_Dynamics(
    ...
    , md_update_gaugefield(0)
    , gauge_force(0)
    , gauge_force_tlsym(0)
    , fermion_force(0)
    , fermion_force_eo_0(0)
    , fermion_force_eo_1(0)
    , fermion_force_eo_2(0)
    , fermion_force_eo_3(0)
    ...
    , gauge_force_tlsym_5(0)
    , gauge_force_tlsym_6(0)
    , gauge_force_tlsym_tmp(
          use_multipass_gauge_force_tlsym(device)
              ? new hardware::buffers::Matrix3x3(NDIM * get_vol4d(device->getLocalLatticeMemoryExtents()), device)
              : 0)
{
    fill_kernels();
}
