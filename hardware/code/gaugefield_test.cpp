...
const Matrixsu3* createGaugefield()
{
	Matrixsu3 * tmp = new Matrixsu3[4*4*4*4*NDIM];
	set_cold(tmp, 4*4*4*4*NDIM);
	return tmp;
}

class GaugefieldTester : public KernelTester {
public:
		GaugefieldTester(std::string kernelName, std::vector<std::string> parameterStrings, int numberOfValues = 1):
			KernelTester(kernelName, parameterStrings, numberOfValues) {
		gaugefieldBuffer = new hardware::buffers::SU3( 4*4*4*4* NDIM, device);
		const Matrixsu3 * gf_host = createGaugefield();
...
BOOST_AUTO_TEST_SUITE ( PLAQUETTE )
	...
	public:
			PlaquetteTester(std::vector<std::string> parameterStrings, int typeOfPlaquette = 1):
				GaugefieldTester("plaquette", parameterStrings, 1), typeOfPlaquette(typeOfPlaquette) {
	...
	BOOST_AUTO_TEST_CASE( PLAQUETTE_1 )
	{
		...
		PlaquetteTester plaquetteTester(parameterStrings, 1);
	}
	...
	BOOST_AUTO_TEST_CASE( PLAQUETTE_TEMPORAL_1 )
	{
		...
		PlaquetteTester plaquetteTester(parameterStrings, 2);
	}
	...
	{
		std::vector<std::string> parameterStrings {"--startcondition=cold", "--nspace=4", "--ntime=4", "--fermact=TWISTEDMASS", "--gaugeact=wilson", "--beta=5.69", "--solver_prec=1e-8", "--test_ref_val=768"};
		PlaquetteTester plaquetteTester(parameterStrings, 3);
	}
...
BOOST_AUTO_TEST_SUITE ( POLYAKOV )
	...
	public:
			PolyakovloopTester(std::vector<std::string> parameterString):
				GaugefieldTester("polyakov", parameterString, 2) {
	...
	BOOST_AUTO_TEST_CASE( POLYAKOV_1 )
	{
		...
		PolyakovloopTester polyakovloopTester(parameterStrings);
	}
...
BOOST_AUTO_TEST_SUITE ( STOUT_SMEAR )
	...
	public:
			StoutSmearTester(std::vector<std::string> parameterString):
				GaugefieldTester("stout_smear", parameterString) {
	...
	BOOST_AUTO_TEST_CASE( STOUT_SMEAR_1 )
	{
		...
		StoutSmearTester StoutSmearTester(parameterStrings);
	}
...
BOOST_AUTO_TEST_SUITE ( RECTANGLES )
	...
	public:
			RectanglesTester(std::vector<std::string> parameterString):
				GaugefieldTester("rectangles", parameterString) {
	...
	BOOST_AUTO_TEST_CASE( RECTANGLES_1 )
	{
		...
		RectanglesTester rectanglesTester(parameterStrings);
	}
