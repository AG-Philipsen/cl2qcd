...
struct SmearingTestParameters : public GaugefieldTestParameters {
    ...
    SmearingTestParameters(const LatticeExtents latticeExtentsIn, GaugefieldFillType fillTypeIn, int rhoIterIn,
                           double rhoIn)
        : TestParameters(latticeExtentsIn)
        , GaugefieldTestParameters(latticeExtentsIn, fillTypeIn)
        , rhoIter(rhoIterIn)
        , rho(rhoIn)
    {
    }
...
struct PlaquetteTester : public GaugefieldTester {
    PlaquetteTester(const ParameterCollection& parameterCollection, const GaugefieldTestParameters testParams,
                    const ReferenceValues rV, const TypeOfPlaquette typeOfPlaquette)
...
struct PolyakovloopTester : public GaugefieldTester {
    PolyakovloopTester(const ParameterCollection& parameterCollection, const GaugefieldTestParameters testParams,
...
struct StoutSmearTester : public GaugefieldTester {
    StoutSmearTester(const ParameterCollection& parameterCollection, struct SmearingTestParameters testParams,
                     const ReferenceValues rV)
...
void performTest2(const ReferenceValues refValuesIn, const LatticeExtents latticeExtentsIn,
                  const GaugefieldFillType fillType, const TypeOfPlaquette plaquetteType)
{
...
void performTest(const ReferenceValues refValuesIn, const LatticeExtents latticeExtentsIn,
                 const GaugefieldFillType fillType, const int rhoIter, const double rho)
{
    ...
    hardware::code::OpenClKernelParametersMockup kernelParameters(testParams.ns, testParams.nt, testParams.rhoIter,
                                                                  testParams.rho, useSmearing);
    ParameterCollection parameterCollection(hardwareParameters, kernelParameters);
    StoutSmearTester tester(parameterCollection, testParams, refValuesIn);
}

void testPlaquette(const ReferenceValues rV, const LatticeExtents lE, const GaugefieldFillType gF,
                   const TypeOfPlaquette pT)
{
...
BOOST_AUTO_TEST_SUITE(PLAQUETTE)
    ...
    {
        testPlaquette(ReferenceValues{1536.}, LatticeExtents{ns4, nt4}, GaugefieldFillType::cold,
                      TypeOfPlaquette::plaquette);
    }

    BOOST_AUTO_TEST_CASE(PLAQUETTE_2)
    {
        testPlaquette(ReferenceValues{1536.002605}, LatticeExtents{ns4, nt4}, GaugefieldFillType::nonTrivial,
                      TypeOfPlaquette::plaquette);
    }
    ...
    {
        testPlaquette(ReferenceValues{24576}, LatticeExtents{ns8, nt8}, GaugefieldFillType::cold,
                      TypeOfPlaquette::plaquette);
    }

    BOOST_AUTO_TEST_CASE(PLAQUETTE_REDUCTION_2)
    {
        testPlaquette(ReferenceValues{124416}, LatticeExtents{ns12, nt12}, GaugefieldFillType::cold,
                      TypeOfPlaquette::plaquette);
    }

    BOOST_AUTO_TEST_CASE(PLAQUETTE_REDUCTION_3)
    {
        testPlaquette(ReferenceValues{393216}, LatticeExtents{ns16, nt16}, GaugefieldFillType::cold,
                      TypeOfPlaquette::plaquette);
    }
...
BOOST_AUTO_TEST_SUITE(CONVERT_GAUGEFIELD_TO_SOA)
    ...
    BOOST_AUTO_TEST_CASE(CONVERT_GAUGEFIELD_TO_SOA_1) { BOOST_TEST_MESSAGE("NOT YET IMPLEMENTED!"); }

BOOST_AUTO_TEST_SUITE_END()

BOOST_AUTO_TEST_SUITE(CONVERT_GAUGEFIELD_FROM_SOA)

    BOOST_AUTO_TEST_CASE(CONVERT_GAUGEFIELD_FROM_SOA_1) { BOOST_TEST_MESSAGE("NOT YET IMPLEMENTED!"); }
