...
void hardware::code::Correlator::fill_kernels()
{
    ...
    if (kernelParameters->getSourceType() == common::point)
        create_point_source = createKernel("create_point_source")
                              << basic_correlator_code << prng_code << "spinorfield_point_source.cl";
    else if (kernelParameters->getSourceType() == common::volume)
        create_volume_source = createKernel("create_volume_source")
                               << basic_correlator_code << prng_code << "spinorfield_volume_source.cl";
    else if (kernelParameters->getSourceType() == common::timeslice)
        create_timeslice_source = createKernel("create_timeslice_source")
                                  << basic_correlator_code << prng_code << "spinorfield_timeslice_source.cl";
    else if (kernelParameters->getSourceType() == common::zslice)
        create_zslice_source = createKernel("create_zslice_source")
                               << basic_correlator_code << prng_code << "spinorfield_zslice_source.cl";
    ...
    if (kernelParameters->getMeasureCorrelators()) {
        ...
        if (kernelParameters->getSourceType() == common::point) {
            switch (kernelParameters->getCorrDir()) {
                case 0:
                    correlator_ps = createKernel("correlator_ps_t")
                                    << basic_correlator_code << "fermionobservables/correlator_ps_point.cl";
                    correlator_sc = createKernel("correlator_sc_t")
                                    << basic_correlator_code << "fermionobservables/correlator_sc_point.cl";
                    correlator_vx = createKernel("correlator_vx_t")
                                    << basic_correlator_code << "fermionobservables/correlator_vx_point.cl";
                    correlator_vy = createKernel("correlator_vy_t")
                                    << basic_correlator_code << "fermionobservables/correlator_vy_point.cl";
                    correlator_vz = createKernel("correlator_vz_t")
                                    << basic_correlator_code << "fermionobservables/correlator_vz_point.cl";
                    correlator_ax = createKernel("correlator_ax_t")
                                    << basic_correlator_code << "fermionobservables/correlator_ax_point.cl";
                    correlator_ay = createKernel("correlator_ay_t")
                                    << basic_correlator_code << "fermionobservables/correlator_ay_point.cl";
                    correlator_az = createKernel("correlator_az_t")
                                    << basic_correlator_code << "fermionobservables/correlator_az_point.cl";
                    correlator_avps = createKernel("correlator_avps_t")
                                      << basic_correlator_code << "fermionobservables/correlator_avps_point.cl";
                    break;
                case 3:
                    correlator_ps = createKernel("correlator_ps_z")
                                    << basic_correlator_code << "fermionobservables/correlator_ps_point.cl";
                    correlator_sc = createKernel("correlator_sc_z")
                                    << basic_correlator_code << "fermionobservables/correlator_sc_point.cl";
                    correlator_vx = createKernel("correlator_vx_z")
                                    << basic_correlator_code << "fermionobservables/correlator_vx_point.cl";
                    correlator_vy = createKernel("correlator_vy_z")
                                    << basic_correlator_code << "fermionobservables/correlator_vy_point.cl";
                    correlator_vz = createKernel("correlator_vz_z")
                                    << basic_correlator_code << "fermionobservables/correlator_vz_point.cl";
                    correlator_ax = createKernel("correlator_ax_z")
                                    << basic_correlator_code << "fermionobservables/correlator_ax_point.cl";
                    correlator_ay = createKernel("correlator_ay_z")
                                    << basic_correlator_code << "fermionobservables/correlator_ay_point.cl";
                    correlator_az = createKernel("correlator_az_z")
                                    << basic_correlator_code << "fermionobservables/correlator_az_point.cl";
                    correlator_avps = 0;
                    break;
                default:
                    stringstream errmsg;
                    errmsg << "Could not create correlator kernel as correlator direction "
                           << kernelParameters->getCorrDir() << " has not been implemented.";
...
void hardware::code::Correlator::get_work_sizes(const cl_kernel kernel, size_t* ls, size_t* gs,
...
void hardware::code::Correlator::create_point_source_device(const hardware::buffers::Plain<spinor>* inout, int i,
                                                            int spacepos, int timepos) const
{
...
void hardware::code::Correlator::create_volume_source_device(const hardware::buffers::Plain<spinor>* inout,
...
void hardware::code::Correlator::create_timeslice_source_device(const hardware::buffers::Plain<spinor>* inout,
                                                                const hardware::buffers::PRNGBuffer* prng,
                                                                const int timeslice) const
{
    // query work-sizes for kernel
    size_t ls2, gs2;
    cl_uint num_groups;
...
void hardware::code::Correlator::create_zslice_source_device(const hardware::buffers::Plain<spinor>* inout,
                                                             const hardware::buffers::PRNGBuffer* prng,
                                                             const int zslice) const
{
    // query work-sizes for kernel
    size_t ls2, gs2;
    cl_uint num_groups;
...
void hardware::code::Correlator::correlator(const cl_kernel correlator_kernel,
                                            const hardware::buffers::Plain<hmc_float>* correlator,
                                            const hardware::buffers::Plain<spinor>* in,
                                            const hardware::buffers::Plain<spinor>* source) const
{
    int clerr;
    // query work-sizes for kernel
    size_t ls2, gs2;
...
void hardware::code::Correlator::correlator(const cl_kernel correlator_kernel,
                                            const hardware::buffers::Plain<hmc_float>* correlator,
                                            const hardware::buffers::Plain<spinor>* in1,
                                            const hardware::buffers::Plain<spinor>* in2,
                                            const hardware::buffers::Plain<spinor>* in3,
                                            const hardware::buffers::Plain<spinor>* in4) const
{
    int clerr;
    // query work-sizes for kernel
    size_t ls2, gs2;
...
void hardware::code::Correlator::correlator(
    const cl_kernel correlator_kernel, const hardware::buffers::Plain<hmc_float>* correlator,
    const hardware::buffers::Plain<spinor>* in1, const hardware::buffers::Plain<spinor>* source1,
    const hardware::buffers::Plain<spinor>* in2, const hardware::buffers::Plain<spinor>* source2,
    const hardware::buffers::Plain<spinor>* in3, const hardware::buffers::Plain<spinor>* source3,
    const hardware::buffers::Plain<spinor>* in4, const hardware::buffers::Plain<spinor>* source4) const
{
    int clerr;
    // query work-sizes for kernel
    size_t ls2, gs2;
...
size_t hardware::code::Correlator::get_read_write_size(const std::string& in) const
{
    ...
    if (in == "correlator_ps_z") {
        ...
        if (kernelParameters->getCorrDir() == 3)
            size_buffer = kernelParameters->getNs();
...
hardware::code::Correlator::Correlator(const hardware::code::OpenClKernelParametersInterface& kernelParameters,
                                       const hardware::Device* device)
    : Opencl_Module(kernelParameters, device)
    , create_point_source(0)
    , create_volume_source(0)
    , create_timeslice_source(0)
    , create_zslice_source(0)
    , correlator_ps(0)
    , correlator_sc(0)
    , correlator_vx(0)
    , correlator_vy(0)
