...
{
    return ReferenceValues{(alpha.re * beta.re + alpha.im * beta.im) / (beta.re * beta.re + beta.im * beta.im),
                           (alpha.im * beta.re - alpha.re * beta.im) / (beta.re * beta.re + beta.im * beta.im)};
}
...
struct ComplexTester : public KernelTester {
    ComplexTester(std::string kernelName, const ParameterCollection pC, const ComplexTestParameters tP,
...
BOOST_AUTO_TEST_SUITE(CONVERT)
    ...
    BOOST_AUTO_TEST_CASE(CONVERT_1) { testConvert(nonTrivialParameter); }
