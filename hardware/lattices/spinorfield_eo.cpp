...
std::vector<const hardware::buffers::Spinor*> hardware::lattices::Spinorfield_eo::allocate_buffers()
{
    ...
    for (auto device : devices) {
        buffers.push_back(
            new Spinor(hardware::code::get_eoprec_spinorfieldsize(device->getLocalLatticeMemoryExtents()), device));
    }
    return buffers;
}
...
void hardware::lattices::Spinorfield_eo::require_halo(unsigned reqd_width) const
{
    ...
    logger.debug() << "Halo of Spinorfield_eo " << this << " required with width " << reqd_width
                   << ". Width of valid halo: " << valid_halo_width;
...
hardware::lattices::Spinorfield_eo::require_halo_async(unsigned reqd_width) const
{
#ifdef LAZY_HALO_UPDATES
    if (!reqd_width) {
        reqd_width = buffers[0]->get_device()->getHaloExtent();
    }
    logger.debug() << "Async Halo of Spinorfield_eo " << this << " required with width " << reqd_width
                   << ". Width of valid halo: " << valid_halo_width;
    if (valid_halo_width < reqd_width) {
