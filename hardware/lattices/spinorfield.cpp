...
std::vector<const hardware::buffers::Plain<spinor>*> hardware::lattices::Spinorfield::allocate_buffers()
{
    ...
    for (auto device : system.get_devices()) {
        buffers.push_back(new Plain<spinor>(hardware::code::get_spinorfieldsize(device->getLocalLatticeMemoryExtents()),
                                            device, place_on_host));
    }
    return buffers;
}
...
void hardware::lattices::Spinorfield::import(const spinor* const host) const
{
    ...
    if (buffers.size() == 1) {
        ...
        for (auto const buffer : buffers) {
            ...
            buffer->load(&host[tmp2.getSecondHaloPartIndex_source()], tmp2.getHaloPartSize(),
                         tmp2.getMainPartSize() + tmp2.getHaloPartSize());
        }
    }
