...
std::vector<const hardware::buffers::SU3vec*> hardware::lattices::Staggeredfield_eo::allocate_buffers()
{
    ...
    for (auto device : devices) {
        buffers.push_back(
            new SU3vec(hardware::code::get_eoprec_spinorfieldsize(device->getLocalLatticeMemoryExtents()), device));
    }
    return buffers;
}
...
hardware::lattices::Staggeredfield_eo::Staggeredfield_eo(hardware::lattices::Staggeredfield_eo&& movedFrom)
    : system(std::move(movedFrom.system))
    ,  // use move even though a copy is done (it is a const reference!)
