...
std::vector<const hardware::buffers::SU3*> hardware::lattices::Gaugefield::allocate_buffers()
{
    ...
    for (auto device : devices) {
        buffers.push_back(new SU3(device->getLocalLatticeMemoryExtents().getLatticeVolume() * 4,
                                  device));  // todo: do not calculate here!
    }
    return buffers;
}
...
void hardware::lattices::Gaugefield::fetch_gaugefield_from_buffers(Matrixsu3* const gf_host)
{
    ...
    if (buffers.size() == 1) {
        ...
        for (auto const buffer : buffers) {
            ...
            TemporalParallelizationHandlerLink tmp2(device->getGridPos(), device->getLocalLatticeExtents(),
                                                    sizeof(Matrixsu3), device->getHaloExtent());
            memcpy(&gf_host[tmp2.getMainPartIndex_source()], &mem_host[tmp2.getMainPartIndex_destination()],
                   tmp2.getMainPartSizeInBytes());
...
void hardware::lattices::Gaugefield::update_halo_aos(const std::vector<const hardware::buffers::SU3*> buffers,
...
void hardware::lattices::Gaugefield::update_halo_soa(const std::vector<const hardware::buffers::SU3*> buffers,
                                                     const hardware::System& system) const
{
    // check all buffers are non-soa
    for (auto const buffer : buffers) {
...
static Matrixsu3 random_matrixsu3()
{
    ...
    c[0].re = (a[1].re * b[2].re - a[1].im * b[2].im) - (a[2].re * b[1].re - a[2].im * b[1].im);
    c[0].im = -(a[1].re * b[2].im + a[1].im * b[2].re) + (a[2].re * b[1].im + a[2].im * b[1].re);

    c[1].re = (a[2].re * b[0].re - a[2].im * b[0].im) - (a[0].re * b[2].re - a[0].im * b[2].im);
    c[1].im = -(a[2].re * b[0].im + a[2].im * b[0].re) + (a[0].re * b[2].im + a[0].im * b[2].re);

    c[2].re = (a[0].re * b[1].re - a[0].im * b[1].im) - (a[1].re * b[0].re - a[1].im * b[0].im);
    c[2].im = -(a[0].re * b[1].im + a[0].im * b[1].re) + (a[1].re * b[0].im + a[1].im * b[0].re);
...
void hardware::lattices::Gaugefield::smear(unsigned int smearingSteps)
{
    ...
    for (size_t i = 0; i < buffers.size(); ++i) {
        ...
        auto gf_code = device->getGaugefieldCode();  // should be like: get_gaugefield_code(
                                                     // HardwareParameters_gaugefield )
