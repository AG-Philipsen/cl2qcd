...
namespace hardware {
    namespace lattices {
        ...
        class Vector {
          public:
            Vector(const size_t N, const hardware::System& system)
                : N(N), system(system), buffers(create_vector_buffers<SCALAR>(N, system)){};
...
template<typename SCALAR>
static std::vector<const hardware::buffers::Plain<SCALAR>*>
hardware::lattices::create_vector_buffers(const size_t N, const hardware::System& system)
{
...
template<typename SCALAR>
std::vector<SCALAR> hardware::lattices::Vector<SCALAR>::get() const
{
...
template<typename SCALAR>
void hardware::lattices::Vector<SCALAR>::store(const std::vector<SCALAR> vec) const
{
    if (vec.size() != N)
...
template<typename SCALAR>
const std::vector<const hardware::buffers::Plain<SCALAR>*> hardware::lattices::Vector<SCALAR>::get_buffers() const
    noexcept
{
    return buffers;
}
...
template<typename SCALAR>
size_t hardware::lattices::Vector<SCALAR>::get_vector_size() const noexcept
{
    return N;
}
