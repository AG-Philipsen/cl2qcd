...
const std::string CACHE_DIR_NAME("cl2qcd/ocl_cache");
...
TmpClKernel::operator cl_kernel() const
{
    ...
    if (clerr != CL_SUCCESS)
        throw Opencl_Error(clerr, "clCreateKernel", __FILE__, __LINE__);
...
void TmpClKernel::printResourceRequirements(const cl_kernel kernel) const
{
    ...
    clerr = clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_COMPILE_WORK_GROUP_SIZE, 3 * sizeof(size_t),
    ...
    if (clerr != CL_SUCCESS) {
        ...
        if (compile_work_group_size[0] == 0)
            ...
            logger.trace() << "  Compile time work group size: (" << compile_work_group_size[0] << ", "
    ...
    clerr = clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_PREFERRED_WORK_GROUP_SIZE_MULTIPLE, sizeof(size_t),
    ...
    clerr = clGetKernelWorkGroupInfo(kernel, device, CL_KERNEL_PRIVATE_MEM_SIZE, sizeof(cl_ulong), &private_mem_size,
                                     NULL);
    if (clerr != CL_SUCCESS) {
        if (clerr == CL_INVALID_VALUE) {
    ...
    if (strcmp("Advanced Micro Devices, Inc.", platform_vendor) == 0 && device_type == CL_DEVICE_TYPE_GPU) {
        // get device name
        ...
        if (boost::regex_search(isa, what, exScratch)) {
            ...
            logger.debug() << "Kernel: " << kernelName << " - " << gp_regs << " GPRs, " << scratch_regs
                           << " scratch registers, " << static_local_bytes
                           << " bytes statically allocated local memory";
        } else {
            ...
            if (boost::regex_search(isa, what, exStatic)) {
                ...
                logger.trace() << "Static local memory allocation section not found. This is expected if no local "
                                  "memory is statically allocated.";
            }

            logger.debug() << "Kernel: " << kernelName << " - " << sgp_regs << " sGPRs, " << vgp_regs << " vGPRs, "
                           << scratch_regs << " scratch registers, " << static_local_bytes
                           << " bytes statically allocated local memory";
        }
        delete[] device_name;
    } else {
...
std::string TmpClKernel::generateMD5() const
{
    ...
    //  * Device (Name + Driver Version): For simplicity assume we only compile for one device at a time (could also
    //  just add all devices)
...
void TmpClKernel::dumpBinary(cl_program program, std::string md5) const
{
    ...
    clerr = clGetProgramInfo(program, CL_PROGRAM_BINARY_SIZES, sizeof(size_t) * num_devices, program_binaries_bytes,
                             NULL);
    if (clerr) {
        throw Opencl_Error(clerr);
    }
    ...
    fs::create_directories(
        binaryfile.parent_path());  // make sure the directory exists, otherwise we cannot open the path for writing
...
cl_program TmpClKernel::loadSources() const
{
    ...
    for (size_t n = 0; n < files.size(); n++) {
        ...
        if (!file.is_open())
            throw File_Exception(filename.string());
    ...
    if (clerr != CL_SUCCESS)
        throw Opencl_Error(clerr, "clCreateProgramWithSource", __FILE__, __LINE__);
...
TmpClKernel::TmpClKernel(const std::string kernel_name, const std::string build_options, const cl_context context,
                         cl_device_id device, const std::vector<std::string> files)
    : kernel_name(kernel_name)
    , build_options(boost::trim_copy(build_options))
    , context(context)
    , device(device)
    , files(files)
{
...
std::string getDirectoryBasedOnExtension(std::string filename)
{
    ...
    } else if (fileExtension.string().compare(HEADER_EXTENSION) == 0 ||
               fileExtension.string().compare(HEADER_EXTENSION2) == 0) {
        return HEADER_DIRECTORY;
    } else {
        std::string errorMessage = "The file " + filename +
                                   " does not seem to have an extension known to the opencl compiler. Aborting...";
