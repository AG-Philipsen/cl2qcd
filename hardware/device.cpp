...
hardware::Device::Device(cl_context context, cl_device_id device_id, LatticeGridIndex lI, LatticeGrid lG,
                         const hardware::OpenClCode& builderIn,
                         const hardware::HardwareParametersInterface& parametersIn)
    : DeviceInfo(device_id)
    , openClCodeBuilder(&builderIn)
    , hardwareParameters(&parametersIn)
    , context(context)
    , profiling_data()
    ...
    , localLatticeExtents(
          LocalLatticeExtents(LatticeExtents(hardwareParameters->getNs(), hardwareParameters->getNt()), lG))
    , haloSize(2)
    ...
    , allocated_hostptr_bytes(0)
{
    ...
    command_queue = clCreateCommandQueue(context, device_id,
                                         hardwareParameters->enableProfiling() ? CL_QUEUE_PROFILING_ENABLE : 0, &err);
    if (err) {
        throw OpenclException(err, "clCreateCommandQueue", __FILE__, __LINE__);
    }

    logger.trace() << "Initial memory usage (" << latticeGridIndex.x << "," << latticeGridIndex.y << ","
...
hardware::Device::~Device()
{
    ...
    logger.info() << "Maximum memory used (" << latticeGridIndex.x << "," << latticeGridIndex.y << ","
                  << latticeGridIndex.z << "," << latticeGridIndex.t << "): " << max_allocated_bytes << " bytes";
}
...
void hardware::Device::enqueue_kernel(cl_kernel kernel, size_t global_threads, size_t local_threads) const
{
    ...
    // queue kernel
    cl_int clerr = clEnqueueNDRangeKernel(command_queue, kernel, 1, 0, &global_threads, &local_threads, 0, 0,
                                          profiling_event_p);
...
#else
#    pragma message "Using legacy function clEnqueueWaitForEvents, " \
                    "upgrade to OpenCL 1.2 to enable the usage of clEnqueueBarrierWithWaitList()."
...
void hardware::Device::enqueue_barrier(const hardware::SynchronizationEvent& event1,
                                       const hardware::SynchronizationEvent& event2) const
{
...
static int get_cypress_stride_badness(size_t bytes, size_t lanes)
{
    ...
    for (size_t hops = 1; hops < lanes; ++hops) {
        ...
        if (dist_to_critical >= (CRITICAL_STRIDE - CRITICAL_STRIDE_RANGE) ||
            dist_to_critical <= CRITICAL_STRIDE_RANGE) {
            ++badness;
        }
    }
    return badness;
    ...
    if (name == std::string("Cypress") || name == std::string("Cayman")) {
        ...
        for (size_t stride = elems; stride <= elems + MAX_ADD_STRIDE / type_size; ++stride) {
            if (get_cypress_stride_badness(stride * type_size, lane_count) == 0) {
                logger.debug() << "Return stride of " << stride << " elements, which is " << stride * type_size
                               << " bytes.";
                return stride;
            }
        }
