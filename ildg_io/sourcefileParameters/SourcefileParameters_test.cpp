...
BOOST_AUTO_TEST_CASE(initFromParameters)
{
    ...
    Checksum checksum(1, 2);
    const char* _params[] = {"foo",        "--nTime=41", "--nSpace=65", "--kappa=-12.345", "--precision=32",
                             "--beta=4.5", "--mu=23.41"};
...
BOOST_AUTO_TEST_CASE(checkAgainstParameters_exception)
{
    ...
    std::string hmcVersion = "3.95";
    Checksum checksum(1, 2);
    const char* _params[]  = {"foo",        "--nTime=41",      "--nSpace=65", "--precision=32",
...
BOOST_AUTO_TEST_CASE(checkAgainstChecksum_exceptionByParameters)
{
    ...
    BOOST_REQUIRE_THROW(values.checkAgainstChecksum(checksum, parameters.get_ignore_checksum_errors(), "nameOfFile"),
                        File_Exception);
}
