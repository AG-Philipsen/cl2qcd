...
// TODO: Remove all occurences of meta::Inputparameters and Inputparameters. The tests should only use
// IlgdIoParameters_gaugefield!!
...
Sourcefileparameters setSourceFileParametersToSpecificValuesForGaugefield()
{
    ...
    srcFileParams.num_entries    = getElementsOfGaugefield(srcFileParams.lx, srcFileParams.ly, srcFileParams.lz,
                                                        srcFileParams.lt);
...
// not implemented are fermion parameters: solvertype_source, hmcversion_solver_source, flavours_source, noiter_source,
// kappa_solver_source, mu_solver_source, epssq_source
...
void writeEmptyGaugefieldFromSourcefileParameters(const meta::Inputparameters* parameters,
...
BOOST_AUTO_TEST_SUITE(conversionToAndFromIldgFormat)
    ...
    BOOST_AUTO_TEST_CASE(conversionToAndFromIldgFormat_global_constant) { fillAndConvert(Matrixsu3_utilities::ONE); }

    BOOST_AUTO_TEST_CASE(conversionToAndFromIldgFormat_global_random) { fillAndConvert(Matrixsu3_utilities::RANDOM); }
                ...
                for (uint z = 0; z < nspace; z++) {
                    for (uint t = 0; t < ntime; t++) {
                        for (uint mu = 0; mu < NDIM; mu++) {
                            positionToSet = uint(LinkIndex(Index(x, y, z, t, LatticeExtents(nspace, ntime)),
                                                           static_cast<Direction>(mu)));
                            gaugefield.setSpecificEntry(Matrixsu3_utilities::getUnitMatrix(), positionToSet);

                            hmc_complex sumBeforeConversion = Matrixsu3_utilities::sumUpAllMatrixElements(
                                gaugefield.getField());
                            checkSumForOneDiagonalMatrix(sumBeforeConversion);

                            convertGaugefieldToAndFromIldg(gaugefield);

                            hmc_complex sumAfterConversion = Matrixsu3_utilities::sumUpAllMatrixElements(
                                gaugefield.getField());
    ...
    BOOST_AUTO_TEST_CASE(conversionToAndFromIldgFormat_specific_1) { testConversion(3, 3); }

    BOOST_AUTO_TEST_CASE(conversionToAndFromIldgFormat_specific_2) { testConversion(3, 5); }

    BOOST_AUTO_TEST_CASE(conversionToAndFromIldgFormat_specific_3) { testConversion(5, 3); }
...
BOOST_AUTO_TEST_SUITE(writeAndRead)
    ...
    BOOST_AUTO_TEST_CASE(writeAndRead_1) { testWriteRead(2, 3); }
    BOOST_AUTO_TEST_CASE(writeAndRead_2) { testWriteRead(3, 2); }
    BOOST_AUTO_TEST_CASE(writeAndRead_3) { testWriteRead(2, 1); }
