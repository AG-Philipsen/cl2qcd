...
{
    return (limeEntryTypes["propagator"] == lime_type || limeEntryTypes["inverter"] == lime_type ||
...
{
    return (limeEntryTypes["scidac binary data"] == lime_type || limeEntryTypes["ildg binary data"] == lime_type)
               ? true
               : false;
}
                                                    ...
                                                    size_t expectedNumberOfBytes)
{
    if (checkLimeEntryForBinaryData(limeHeaderData.limeEntryType, limeEntryTypes)) {
        if (limeFileProp.numberOfBinaryDataEntries == 1) {
            ...
            limeReaderReadData(destination[limeFileProp.numberOfBinaryDataEntries - 1], &limeHeaderData.numberOfBytes,
                               limeReader);
        } else {
...
void LimeFileReader::extractInformationFromLimeEntry(char** destination, size_t expectedNumberOfBytes)
{
    ...
    if (limeHeaderData.MB_flag != LIME_ERR_MBME) {
        ...
        logger.fatal() << "Error while reading LIME entry. MB flag is \"" +
                              boost::lexical_cast<std::string>(limeHeaderData.MB_flag) + "\"";
    }
}
...
void LimeFileReader::handleLimeEntry_ildg(Sourcefileparameters& parameters, char* buffer, std::string lime_type,
