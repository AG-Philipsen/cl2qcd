...
LatticeGridIndex::LatticeGridIndex(const latticeCoordinate x, const latticeCoordinate y, const latticeCoordinate z,
                                   const latticeCoordinate t, const LatticeGrid lG)
    : Index(x, y, z, t, lG)
{
}
...
LocalLatticeMemoryExtents::LocalLatticeMemoryExtents(const LatticeGrid lG, const LocalLatticeExtents llE,
                                                     unsigned int halo_size)
    : LatticeExtents(llE.xExtent + (lG.xExtent > 1 ? 2 * halo_size : 0),
                     llE.yExtent + (lG.yExtent > 1 ? 2 * halo_size : 0),
                     llE.zExtent + (lG.zExtent > 1 ? 2 * halo_size : 0),
                     llE.tExtent + (lG.tExtent > 1 ? 2 * halo_size : 0))
{
