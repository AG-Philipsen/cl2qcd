...
class iTimer {
  ...
  public:
    ...
    double End()
    {
        ...
        AccumulatedTime += ((double)EndTime.tv_sec + EndTime.tv_usec / 1E6 -
                            ((double)StartTime.tv_sec + StartTime.tv_usec / 1E6));
...
void Compile_OpenCL_Code(std::string BuildOptions, std::string FileName, cl::Context& context, cl::Program& program,
                         cl_uint verbosity)
{
...
void OpenCL_Initializations(cl::Context& context, std::vector<cl::Device>& devices, cl::CommandQueue& queue,
                            bool PrintOpenCLInfo, bool UseOpenCLProfiling, bool UseGPU, cl_uint deviceNr,
                            cl_uint platformNr)
{
    ...
    if (PrintOpenCLInfo == 1) {
        ...
        if (ulong == CL_NONE)
            std::cout << "None\n";
        if (ulong == CL_READ_ONLY_CACHE)
            std::cout << "Read only\n";
        if (ulong == CL_READ_WRITE_CACHE)
            std::cout << "Read/Write cache\n";
...
void CheckCorrectness(size_t NumWorkitems, int lux, cl_int KernelCycles, cl_int NumIterations, cl_float* PRNs)
{
    ...
    size_t NumVals, numRefFileValsPerpval = 51200, NumValsToCheck = NumWorkitems, numBackupValsPerpval = 4, offset = 0,
                    numErrors;
    ...
    // The values of each of the five p-values are stored one after the other. This sets the offset we need to index
    // from.
    if (lux == 0)
        offset = 0;
    else if (lux == 1)
    ...
    if (KernelCycles * NumIterations != 1000000) {
        std::cout << "Correctness check only possible if 1000000 values are generated per work-item, but\n"
                  << KernelCycles * NumIterations
                  << " have been generated. Correctness check will not be performed\n\n";
        doCorrectnessCheck = 0;
    }
    ...
    if (doCorrectnessCheck) {
        ...
        if (refVals == NULL) {
            ...
            refVals[0 + 0 * numBackupValsPerpval] = (cl_float)0.82777202129364;
            refVals[1 + 0 * numBackupValsPerpval] = (cl_float)0.14299058914185;
            refVals[2 + 0 * numBackupValsPerpval] = (cl_float)0.98017895221710;
            refVals[3 + 0 * numBackupValsPerpval] = (cl_float)0.96829599142075;

            refVals[0 + 1 * numBackupValsPerpval] = (cl_float)0.32378226518631;
            refVals[1 + 1 * numBackupValsPerpval] = (cl_float)0.68649446964264;
            refVals[2 + 1 * numBackupValsPerpval] = (cl_float)0.89647895097733;
            refVals[3 + 1 * numBackupValsPerpval] = (cl_float)0.42210841178894;

            refVals[0 + 2 * numBackupValsPerpval] = (cl_float)0.74214434623718;
            refVals[1 + 2 * numBackupValsPerpval] = (cl_float)0.58056205511093;
            refVals[2 + 2 * numBackupValsPerpval] = (cl_float)0.77871483564377;
            refVals[3 + 2 * numBackupValsPerpval] = (cl_float)0.72025460004807;

            refVals[0 + 3 * numBackupValsPerpval] = (cl_float)0.71860218048096;
            refVals[1 + 3 * numBackupValsPerpval] = (cl_float)0.76306450366974;
            refVals[2 + 3 * numBackupValsPerpval] = (cl_float)0.56076055765152;
            refVals[3 + 3 * numBackupValsPerpval] = (cl_float)0.45423328876495;

            refVals[0 + 4 * numBackupValsPerpval] = (cl_float)0.42760473489761;
            refVals[1 + 4 * numBackupValsPerpval] = (cl_float)0.10520881414414;
            refVals[2 + 4 * numBackupValsPerpval] = (cl_float)0.30440622568131;
            refVals[3 + 4 * numBackupValsPerpval] = (cl_float)0.50632297992706;
        } else
            offset *= numRefFileValsPerpval;
        ...
        std::cout << std::setprecision(6) << std::fixed << "Number of errors found in " << NumValsToCheck
                  << " values (should be zero): " << numErrors << "\n"
                  << "Total summed up difference of all values (should be zero): " << diffSum << "\n"
                  << "The averages of the reference and generated numbers (should be equal): " << avgRef << " and "
                  << avgGen << "\n\n";
    }
}
...
int main(int argc, char* argv[])
{
    ...
    int lux = atoi(argv[1]);
    if (argc > 2)
        UseGPU = atoi(argv[2]) ? 1 : 0;
    if (argc > 3)
        ShowBuildLog = atoi(argv[3]) ? 1 : 0;
    if (argc > 4)
        PrintOpenCLInfo = atoi(argv[4]) ? 1 : 0;
    if (argc > 5)
        deviceNr = atoi(argv[5]);
    if (argc > 6)
        platformNr = atoi(argv[6]);
    ...
    // Setting kernels
    cl::Kernel Kernel_RanluxclInit = cl::Kernel(program, "Kernel_Ranluxcl_Init", &err);
    checkErr(err, "cl::Kernel Kernel_RanluxclInit");
    cl::Kernel Kernel_PRN = cl::Kernel(program, "Kernel_PRN", &err);
    checkErr(err, "cl::Kernel kernel_RanluxclInit");
    ...
    err = Kernel_PRN.setArg(0, KernelCycles);
    checkErr(err, "Kernel::setArg()");
    err = Kernel_PRN.setArg(1, Buffer_RANLUXCLTab);
    checkErr(err, "Kernel::setArg()");
    err = Kernel_PRN.setArg(2, Buffer_PRNs);
    checkErr(err, "Kernel::setArg()");

    err = Kernel_RanluxclInit.setArg(0, 0);
    checkErr(err, "Kernel::setArg()");
    err = Kernel_RanluxclInit.setArg(1, Buffer_RANLUXCLTab);
    checkErr(err, "Kernel::setArg()");

    // Initialize the generator
    err = queue.enqueueNDRangeKernel(Kernel_RanluxclInit, cl::NullRange, cl::NDRange(NumWorkitems),
    ...
    for (int Iteration = 0; Iteration < NumIterations; Iteration++) {
        err = queue.enqueueNDRangeKernel(Kernel_PRN, cl::NullRange, cl::NDRange(NumWorkitems),
                                         cl::NDRange(WorkgroupSize));
        checkErr(err, "CommandQueue::enqueueNDRangeKernel()");
    }
    ...
    err = queue.enqueueReadBuffer(Buffer_PRNs, CL_TRUE, 0, NumWorkitems * sizeof(cl_float), PRNs);
    ...
    std::cout << std::setprecision(2) << std::fixed << "Total program time:      " << totalTimer.Time() << " seconds\n"
