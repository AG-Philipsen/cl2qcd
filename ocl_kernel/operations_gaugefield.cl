...
#    ifdef _USE_REC12_
    ...
    return (Matrixsu3){in[0 * GAUGEFIELD_STRIDE + idx],
                       ...
                       complexsubtract(complexmult(in[1 * GAUGEFIELD_STRIDE + idx], in[5 * GAUGEFIELD_STRIDE + idx]),
                                       complexmult(in[2 * GAUGEFIELD_STRIDE + idx], in[4 * GAUGEFIELD_STRIDE + idx])),
                       complexsubtract(complexmult(in[2 * GAUGEFIELD_STRIDE + idx], in[3 * GAUGEFIELD_STRIDE + idx]),
...
inline Matrix3x3 get3x3(__global const Matrix3x3StorageType* const restrict in, const uint idx)
{
    ...
    return (Matrix3x3){in[0 * GAUGEFIELD_3X3_STRIDE + idx], in[1 * GAUGEFIELD_3X3_STRIDE + idx],
                       in[2 * GAUGEFIELD_3X3_STRIDE + idx], in[3 * GAUGEFIELD_3X3_STRIDE + idx],
                       in[4 * GAUGEFIELD_3X3_STRIDE + idx], in[5 * GAUGEFIELD_3X3_STRIDE + idx],
                       in[6 * GAUGEFIELD_3X3_STRIDE + idx], in[7 * GAUGEFIELD_3X3_STRIDE + idx],
 ...
 *  ^+  Udagger_nu(x - nu + mu) * Udagger_nu(x - nu - nu + mu) * Udagger_mu(x - nu - nu) * Ud_nu(x - nu - nu) * U_nu(x -
 * nu)
                                           ...
                                           const int t, const int mu, const int nu)
{
    ...
    tmp = multiply_matrixsu3_dagger_dagger(get_matrixsu3(field, pos2.w, pos2.z, nu),
                                           get_matrixsu3(field, pos.w, pos.z, mu));
    tmp = multiply_matrixsu3_dagger(tmp, get_matrixsu3(field, pos2.y, pos2.x, mu));
                                           ...
                                           const int t, const int mu, const int nu)
{
    ...
    // 4. Udagger_nu(x - nu + mu) * Udagger_nu(x - nu - nu + mu) * Udagger_mu(x - nu - nu) * Ud_nu(x - nu - nu) * U_nu(x
    // - nu) (x, y) = i - nu (call the site idx "x" "i" for now)
    if (nu == 0) {
        pos.x = get_lower_neighbor_temporal(t);
        pos.y = n;
    } else {
    ...
    tmp = multiply_matrixsu3_dagger_dagger(get_matrixsu3(field, pos2.y, pos2.x, nu),
                                           get_matrixsu3(field, pos2.w, pos2.z, nu));
    tmp = multiply_matrixsu3_dagger(tmp, get_matrixsu3(field, pos.w, pos.z, mu));
    tmp = multiply_matrixsu3(tmp, get_matrixsu3(field, pos.w, pos.z, nu));
