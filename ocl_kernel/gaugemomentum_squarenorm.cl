...
__kernel void gaugemomentum_squarenorm(__global const aeStorageType* const restrict in,
                                       __global hmc_float* const restrict out,
                                       __local hmc_float* const restrict result_local)
{
    hmc_float sum = 0.f;
    PARALLEL_FOR (id_local, VOL4D_LOCAL) {
        site_idx site = get_site_idx((id_local % 2 == 0) ? get_even_st_idx_local(id_local / 2)
                                                         : get_odd_st_idx_local(id_local / 2));
        for (uint d = 0; d < 4; ++d) {
    ...
    if (local_size == 1) {
        ...
        if (idx == 0) {
            out[group_id] = result_local[0] + result_local[1] + result_local[2] + result_local[3] + result_local[4] +
