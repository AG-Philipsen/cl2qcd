                        ...
                        __local hmc_float* tplaq_loc, __local hmc_float* splaq_loc)
{
    ...
    for (id_local = id_tmp; id_local < VOL4D_LOCAL; id_local += global_size) {
        st_index pos = (id_local < VOL4D_LOCAL / 2) ? get_even_st_idx_local(id_local)
                                                    : get_odd_st_idx_local(id_local - (VOL4D_LOCAL / 2));
    ...
    if (local_size == 1) {
        ...
        if (idx == 0) {
            plaq_out[group_id] = plaq_loc[0] + plaq_loc[1] + plaq_loc[2] + plaq_loc[3] + plaq_loc[4] + plaq_loc[5] +
                                 plaq_loc[6] + plaq_loc[7];
            tplaq_out[group_id] = tplaq_loc[0] + tplaq_loc[1] + tplaq_loc[2] + tplaq_loc[3] + tplaq_loc[4] +
                                  tplaq_loc[5] + tplaq_loc[6] + tplaq_loc[7];
            splaq_out[group_id] = splaq_loc[0] + splaq_loc[1] + splaq_loc[2] + splaq_loc[3] + splaq_loc[4] +
                                  splaq_loc[5] + splaq_loc[6] + splaq_loc[7];
        }
    }
    return;
}
