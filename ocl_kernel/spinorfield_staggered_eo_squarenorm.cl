...
// Description of variables of global_squarenorm_staggered_eoprec kernel:
//  - x: The staggered field (an su3vec per each eo site => vector of VOL4D/2 components that are su3vec varibles)
...
__kernel void global_squarenorm_staggered_eoprec(__global const staggeredStorageType* const restrict x,
                                                 ...
                                                 __local hmc_float* const restrict result_local)
{
    ...
    if (local_size == 1) {
        ...
        if (idx == 0) {
            result[group_id] = result_local[0] + result_local[1] + result_local[2] + result_local[3] + result_local[4] +
                               result_local[5] + result_local[6] + result_local[7];
        }
    }
    return;
}
