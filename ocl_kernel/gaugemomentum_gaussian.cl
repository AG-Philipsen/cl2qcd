/*
 * Copyright (c) 2011,2013 Christopher Pinke
...
__kernel void generate_gaussian_gaugemomenta(__global aeStorageType* const restrict out,
...
#endif
        site_idx site = get_site_idx(id_local >= (VOL4D_LOCAL / 2) ? get_even_st_idx_local(id_local - (VOL4D_LOCAL / 2))
                                                                   : get_odd_st_idx_local(id_local));
        for (uint d = 0; d < 4; ++d) {
