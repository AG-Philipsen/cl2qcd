...
{
    hmc_float result = (in).e0 * (in).e0 + (in).e1 * (in).e1 + (in).e2 * (in).e2 + (in).e3 * (in).e3 +
                       (in).e4 * (in).e4 + (in).e5 * (in).e5 + (in).e6 * (in).e6 + (in).e7 * (in).e7;
    return result;
}
...
#ifdef GAUGEMOMENTASIZE_MEM  // we only need the ifdef so we can use the non-field functions without having the field
                             // defined
ae getAe(__global const aeStorageType* const restrict in, const size_t idx)
{
#    ifdef _USE_SOA_
    return (ae){in[0 * GAUGEMOMENTA_STRIDE + idx], in[1 * GAUGEMOMENTA_STRIDE + idx], in[2 * GAUGEMOMENTA_STRIDE + idx],
                in[3 * GAUGEMOMENTA_STRIDE + idx], in[4 * GAUGEMOMENTA_STRIDE + idx], in[5 * GAUGEMOMENTA_STRIDE + idx],
                in[6 * GAUGEMOMENTA_STRIDE + idx], in[7 * GAUGEMOMENTA_STRIDE + idx]};
#    else
    return in[idx];
#    endif
}
...
void update_gaugemomentum(const ae in, const hmc_float factor, const int global_link_pos,
