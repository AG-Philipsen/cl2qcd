...
__kernel void create_timeslice_source(__global spinor* const restrict b,
                                      __global rngStateStorageType* const restrict rngStates, int const timeslice)
{
        ...
        for (int t = 0; t < NTIME_LOCAL; t++) {
            if (t != timeslice) {
                ...
                switch (SOURCE_CONTENT) {
                    ...
                    default:
                        if (id == 0)
                            printf("Problem occured in source kernel: Selected sourcecontent not implemented! Fill "
                                   "with zero...\n");
                        out_tmp = set_spinor_zero();
                }
            }
