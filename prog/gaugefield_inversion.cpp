...
hmc_error Gaugefield_inversion::perform_inversion_pointsource_ps_corr_devices(usetimer* copytimer, usetimer* singletimer, usetimer* solvertimer){
  ...
  for(int k=0; k<12; k++) {
    if(use_eo == false){
      ...
      get_devices_fermions()[0].solver_device(M_call, get_devices_fermions()[0].get_clmem_inout(), get_devices_fermions()[0].get_clmem_source(), get_devices_fermions()[0].get_clmem_gaugefield(), copytimer, singletimer, solvertimer, get_parameters()->get_cgmax());
