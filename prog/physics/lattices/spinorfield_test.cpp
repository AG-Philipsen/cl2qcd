...
BOOST_AUTO_TEST_CASE(saxpy)
{
	...
	BOOST_CHECK_CLOSE(physics::lattices::squarenorm(sf), 3121.566404351164, .1);
}
...
BOOST_AUTO_TEST_CASE(saxsbypz)
{
	...
	BOOST_CHECK_CLOSE(physics::lattices::squarenorm(sf), 13610.02952297107, .1);
	physics::lattices::saxsbypz(&sf, {.1, .3}, zero, {.7, .3}, cold, cold);
	BOOST_CHECK_CLOSE(physics::lattices::squarenorm(sf), 18604.535769932936, .1);
}
