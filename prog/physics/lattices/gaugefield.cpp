...
static void update_halo_soa(const std::vector<const hardware::buffers::SU3 *> buffers, const meta::Inputparameters& params)
{
	...
	const unsigned HALO_CHUNK_ELEMS = HALO_SIZE * VOLSPACE / 2;
	...
	for(size_t i = 0; i < num_buffers; ++i) {
		upper_boundaries.push_back(new char[HALO_ELEMS * sizeof(Matrixsu3)]);
		lower_boundaries.push_back(new char[HALO_ELEMS * sizeof(Matrixsu3)]);
	}
	...
	for(size_t i = 0; i < num_buffers; ++i) {
		...
		extract_boundary(upper_boundaries[i], buffer, (VOL4D_LOCAL / 2) - HALO_CHUNK_ELEMS, HALO_CHUNK_ELEMS);
	...
	for(size_t i = 0; i < num_buffers; ++i) {
		...
		send_halo(buffer, upper_boundaries[lower_grid_neighbour(i, GRID_SIZE)], (VOL4D_LOCAL / 2)  + HALO_CHUNK_ELEMS, HALO_CHUNK_ELEMS);
		// our upper halo is the lower bounary of our upper neighbour, it's stored right after our local data
		send_halo(buffer, lower_boundaries[upper_grid_neighbour(i, GRID_SIZE)], VOL4D_LOCAL / 2, HALO_CHUNK_ELEMS);
	}
...
static void extract_boundary(char* host, const hardware::buffers::SU3 * buffer, size_t in_lane_offset, size_t HALO_CHUNK_ELEMS)
{
	...
	const unsigned VOL4D_MEM = get_vol4d(buffer->get_device()->get_mem_lattice_size());
	const unsigned CHUNKS_PER_LANE = 2 * NDIM;
	const unsigned CHUNK_STRIDE = VOL4D_MEM / 2;
