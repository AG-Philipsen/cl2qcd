...
void Gaugefield_hmc::perform_hmc_step(int dev, inputparameters *parameters, hmc_observables *obs, int iter, hmc_float rnd_number)
{
	...
	get_devices_hmc()[dev].copy_buffer_on_device(get_devices_hmc()[dev].get_clmem_gaugefield(), get_devices_hmc()[dev].get_clmem_new_u(), NDIM * parameters->get_volspace() * parameters->get_nt() * sizeof(Matrixsu3));
	...
	*obs = get_devices_hmc()[dev].metropolis(rnd_number, (*parameters).get_beta());
	...
	if((*obs).accept == 1) {
		...
		get_devices_hmc()[dev].copy_buffer_on_device(get_devices_hmc()[dev].get_clmem_new_u(), get_devices_hmc()[dev].get_clmem_gaugefield(), NDIM * parameters->get_volspace() * parameters->get_nt() * sizeof(Matrixsu3));
