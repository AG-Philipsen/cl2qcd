...
hmc_error Opencl_k::run_kappa_karsch_gpu(const hmc_float beta, usetimer * const timer, hmc_float * kappa_karsch_out)
{
	...
	//read out values
	clerr = clEnqueueReadBuffer(queue, clmem_kappa_karsch, CL_FALSE, 0, sizeof(hmc_float), kappa_karsch_out, 0, NULL, NULL);
	if(clerr != CL_SUCCESS) {
		cout << "... failed, aborting." << endl;
		exit(HMC_OCLERROR);
	}
...
hmc_error Opencl_k::run_kappa_clover_gpu(const hmc_float beta, usetimer * const timer, hmc_float * kappa_clover_out)
{
	...
	//read out values
	clerr = clEnqueueReadBuffer(queue, clmem_kappa_clover, CL_FALSE, 0, sizeof(hmc_float), kappa_clover_out, 0, NULL, NULL);
	if(clerr != CL_SUCCESS) {
		cout << "... failed, aborting." << endl;
		exit(HMC_OCLERROR);
	}
