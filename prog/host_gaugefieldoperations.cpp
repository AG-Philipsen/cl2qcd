...
hmc_error init_gaugefield(hmc_gaugefield* gaugefield,inputparameters* parameters, usetimer* timer)
{
	...
	} else if(parameters->get_startcondition() == COLD_START) {
		timer->reset();
		set_gaugefield_cold(gaugefield);
		timer->add();
	} else if(parameters->get_startcondition() == HOT_START) {
		timer->reset();
		set_gaugefield_cold(gaugefield);
		timer->add();
	} else {
		std::cerr << "Please select a valid start condition." << std::endl;
