...
// //this is the scalar pion correlator in z-direction from pointsources
		...
		for(int t = 0; t < NTIME; t++) {
		  for(int x = 0; x < NSPACE; x++) {
		    for(int y = 0; y < NSPACE; y++) {
		      ...
		      correlator_ps += - su3vec_squarenorm(tmp.e0) - su3vec_squarenorm(tmp.e1) + su3vec_squarenorm(tmp.e2) + su3vec_squarenorm(tmp.e3);
