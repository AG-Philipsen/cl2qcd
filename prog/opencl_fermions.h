...
hmc_error opencl::perform_benchmark(int cgmax, const size_t ls, const size_t gs, usetimer * copytimer, usetimer * singletimer, usetimer * Mtimer, usetimer * scalarprodtimer, usetimer * latimer, usetimer * solvertimer, usetimer * dslashtimer, usetimer * Mdiagtimer){
	hmc_float resid;
	hmc_spinor_field phi[SPINORFIELDSIZE];
	if(!use_eo){
		convert_to_kappa_format_device(clmem_inout, ls, gs, latimer);
