...
/**
 * Set all matrices in the gaugefield gausian distributed random matrices, representing
 * a hot state.
 *
 * @param[out] field Pointer to a field of SU3 matrices
 * @return Error code as defined in hmcerrs.h
...
/**
 * Reduce an SU3 matrix to an SU2
 * @todo is this explanation correct?
 *
 * @param[out] dest SU2 matrix to store to
 * @param[in] src The SU3 matrix to store
 * @param[in] rand Which part of the SU3 to use
 */
void reduction (hmc_complex dest[su2_entries], hmc_staplematrix src, const int rand);
/**
 * Expand an SU2 matrix to an SU3
 * @todo is this explanation correct?
 *
 * @param[out] dest SU3 matrix to store to
 * @param[in] rand Which parts to use
 * @param[in] src The SU2 matrix to extend
 */
void extend (hmc_su3matrix * dest, const int random, hmc_complex src[su2_entries]);
/**
 * @todo What does this do? I assume it ensures the matrix is actually of unit determinant.
 *
...
/**
 * Calculate the part of the polyakov loop local to the given spatial index.
 *
...
/**
 * Calcualte the square norm of the gauge momenta.
 *
