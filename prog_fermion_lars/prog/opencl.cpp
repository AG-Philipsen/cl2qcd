...
hmc_error opencl::testing(){
  ...
  clmem_beta = clCreateBuffer(context,CL_MEM_READ_WRITE,sizeof(hmc_float),0,&clerr);
  clerr = clEnqueueWriteBuffer(queue,clmem_beta,CL_TRUE,0,sizeof(hmc_float),&beta,0,0,NULL);
  if(clerr!=CL_SUCCESS) {
    cout<<"... failed, aborting."<<endl;
    exit(HMC_OCLERROR);
  }
  ...
  clerr = clSetKernelArg(testkernel,1,sizeof(cl_mem),&clmem_beta);
  if(clerr!=CL_SUCCESS) {
    cout<<"clSetKernelArg failed, aborting..."<<endl;
    exit(HMC_OCLERROR);
  }
