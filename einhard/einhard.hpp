/**
 * Copyright (c) 2011,2012 Matthias Bach
...
#define ANSI_COLOR_CLEAR _NoColor::ANSI
    ...
    class OutputFormatter {
      ...
      public:
        OutputFormatter(std::ostream* const out, bool const colorize) : out(out), colorize(colorize), resetColor(false)
        {
            if (out != 0) {
        ...
        template<typename T>
        inline const OutputFormatter<VERBOSITY>& operator<<(const Color<T>& col) const
        {
            if (out && colorize) {
                *out << col.ansiCode();
        ...
        template<typename T>
        inline const OutputFormatter<VERBOSITY>& operator<<(const T& msg) const
        {
        ...
        ~OutputFormatter()
        {
            if (out != 0) {
                ...
                *out << std::resetiosflags(std::ios_base::floatfield | std::ios_base::basefield |
                                           ...
                                           std::ios_base::showpos | std::ios_base::showpoint | std::ios_base::showbase |
    ...
    class Logger {
      ...
      public:
         ...
         * parameter
         */
        inline void setVerbosity(LogLevel verbosity) { this->verbosity = verbosity; }
         ...
         * preffered.
         */
        inline LogLevel getVerbosity() const { return this->verbosity; }
        /**
         * Retrieve a human readable representation of the current log level
         */
        inline char const* getVerbosityString() const { return getLogLevelString(this->verbosity); }
        /**
         * Select whether the output stream should be colorized.
         */
        inline void setColorize(bool colorize) { this->colorize = colorize; }
        /**
         * Check whether the output stream is colorized.
         */
        inline bool getColorize() const { return this->colorize; }
    };
    ...
    template<LogLevel MAX>
    const OutputFormatter<TRACE> Logger<MAX>::trace() const
    {
        if (beTrace())
    ...
    template<LogLevel MAX>
    const OutputFormatter<DEBUG> Logger<MAX>::debug() const
    {
        if (beDebug())
    ...
    template<LogLevel MAX>
    const OutputFormatter<INFO> Logger<MAX>::info() const
    {
        if (beInfo())
    ...
    template<LogLevel MAX>
    bool Logger<MAX>::beInfo() const
    {
        return (MAX <= INFO && verbosity <= INFO);
    }

    template<LogLevel MAX>
    const OutputFormatter<WARN> Logger<MAX>::warn() const
    {
        if (beWarn())
    ...
    template<LogLevel MAX>
    bool Logger<MAX>::beWarn() const
    {
        return (MAX <= WARN && verbosity <= WARN);
    }

    template<LogLevel MAX>
    const OutputFormatter<ERROR> Logger<MAX>::error() const
    {
        if (beError())
    ...
    template<LogLevel MAX>
    bool Logger<MAX>::beError() const
    {
        return (MAX <= ERROR && verbosity <= ERROR);
    }

    template<LogLevel MAX>
    const OutputFormatter<FATAL> Logger<MAX>::fatal() const
    {
        if (beFatal())
...
#define EINHARD_STREAM_T2(T1, T2, arg1, arg2)                                                                          \
    ...
    inline const einhard::OutputFormatter<VERBOSITY>& operator<<(const einhard::OutputFormatter<VERBOSITY>& out, arg1, \
                                                                 arg2)
