...
namespace physics {
    ...
    namespace lattices {
        ...
        void
        subtract(const Scalar<hmc_float>* dest, const Scalar<hmc_float>& minuend, const Scalar<hmc_float>& subtrahend);
        void multiply(const Scalar<hmc_float>* dest, const Scalar<hmc_float>& left, const Scalar<hmc_float>& right);
        void
        divide(const Scalar<hmc_float>* dest, const Scalar<hmc_float>& numerator, const Scalar<hmc_float>& denominator);

        // See hardware/code/real.hpp file for functions documentations
        void update_zeta_cgm(const Vector<hmc_float>* out, const Vector<hmc_float>& zeta_prev,
        ...
        void update_beta_cgm(const Vector<hmc_float>* out, const Scalar<hmc_float>& sbeta_pres,
                             const Vector<hmc_float>& zeta_pres, const Vector<hmc_float>& zeta_prev, const int numeq);

        void update_alpha_cgm(const Vector<hmc_float>* out, const Scalar<hmc_float>& salpha_pres,
                              const Vector<hmc_float>& zeta_pres, const Vector<hmc_float>& beta_pres,
                              const Vector<hmc_float>& zeta_prev, const Scalar<hmc_float>& sbeta_pres, const int numeq);
