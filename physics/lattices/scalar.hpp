...
template<typename SCALAR>
physics::lattices::Scalar<SCALAR>::~Scalar()
{
}

template<typename SCALAR>
SCALAR physics::lattices::Scalar<SCALAR>::get() const
{
    return scalar.get();
}

template<typename SCALAR>
void physics::lattices::Scalar<SCALAR>::sum() const
{
    scalar.sum();
}

template<typename SCALAR>
SCALAR physics::lattices::Scalar<SCALAR>::get_sum() const
{
    return scalar.get_sum();
}

template<typename SCALAR>
void physics::lattices::Scalar<SCALAR>::store(const SCALAR& val) const
{
    scalar.store(val);
}

template<typename SCALAR>
const std::vector<const hardware::buffers::Plain<SCALAR>*> physics::lattices::Scalar<SCALAR>::get_buffers() const
    noexcept
{
