...
physics::lattices::SwappableSpinorfield::SwappableSpinorfield(
    ...
    const physics::lattices::SpinorfieldParametersInterface& spinorfieldParametersInterface, const bool place_on_host)
    : Spinorfield(system, spinorfieldParametersInterface, place_on_host)
{
    // nothing to do
}
...
void physics::lattices::SwappableSpinorfield::swap_in()
{
    ...
    if (num_bufs != swap.size()) {
        throw Print_Error_Message("Spinorfield does not have same structure as its own swap, something went really "
                                  "wrong!",
                                  __FILE__, __LINE__);
    }
...
physics::lattices::create_swappable_spinorfields(const hardware::System& system, const size_t n,
                                                 ...
                                                 const bool place_on_host)
{
    ...
    for (size_t i = 0; i < n; ++i) {
        auto field = new SwappableSpinorfield(system, interfacesHandler.getInterface<physics::lattices::Spinorfield>(),
                                              place_on_host);
        field->swap_out();
