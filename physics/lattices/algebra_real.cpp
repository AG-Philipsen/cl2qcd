...
void physics::lattices::access_real_vector_element(const Scalar<hmc_float>* out, const Vector<hmc_float>& in,
                                                   const int index)
{
...
void physics::lattices::access_real_vector_element(const Vector<hmc_float>* out, const Scalar<hmc_float>& in,
                                                   const int index)
{
    auto out_bufs   = out->get_buffers();
    size_t num_bufs = out_bufs.size();
...
void physics::lattices::add(const Scalar<hmc_float>* dest, const Scalar<hmc_float>& left,
                            const Scalar<hmc_float>& right)
{
...
void physics::lattices::subtract(const Scalar<hmc_float>* dest, const Scalar<hmc_float>& minuend,
                                 const Scalar<hmc_float>& subtrahend)
{
    auto dest_bufs       = dest->get_buffers();
    size_t num_bufs      = dest_bufs.size();
...
void physics::lattices::multiply(const Scalar<hmc_float>* dest, const Scalar<hmc_float>& left,
                                 const Scalar<hmc_float>& right)
{
    auto dest_bufs  = dest->get_buffers();
    size_t num_bufs = dest_bufs.size();
...
void physics::lattices::divide(const Scalar<hmc_float>* dest, const Scalar<hmc_float>& numerator,
                               const Scalar<hmc_float>& denominator)
{
    auto dest_bufs        = dest->get_buffers();
    size_t num_bufs       = dest_bufs.size();
...
void physics::lattices::update_zeta_cgm(const Vector<hmc_float>* out, const Vector<hmc_float>& zeta_prev,
                                        const Vector<hmc_float>& zeta_prev_prev, const Scalar<hmc_float>& sbeta_prev,
                                        const Scalar<hmc_float>& sbeta_pres, const Scalar<hmc_float>& salpha_prev,
                                        const Vector<hmc_float>& sigma, const int numeq)
{
    auto out_bufs            = out->get_buffers();
    size_t num_bufs          = out_bufs.size();
    ...
    for (size_t i = 0; i < num_bufs; ++i) {
        auto code = out_bufs[i]->get_device()->getRealCode();
        code->update_zeta_cgm_device(zeta_prev_bufs[i], zeta_prev_prev_bufs[i], sbeta_prev_bufs[i], sbeta_pres_bufs[i],
                                     salpha_prev_bufs[i], sigma_bufs[i], numeq, out_bufs[i]);
    }
}

void physics::lattices::update_beta_cgm(const Vector<hmc_float>* out, const Scalar<hmc_float>& sbeta_pres,
                                        const Vector<hmc_float>& zeta_pres, const Vector<hmc_float>& zeta_prev,
                                        const int numeq)
{
    auto out_bufs        = out->get_buffers();
    size_t num_bufs      = out_bufs.size();
    ...
    if (num_bufs != zeta_pres_bufs.size() || num_bufs != zeta_prev_bufs.size() || num_bufs != sbeta_pres_bufs.size()) {
        throw std::invalid_argument("All arguments must use the same number of devices.");
    }
...
void physics::lattices::update_alpha_cgm(const Vector<hmc_float>* out, const Scalar<hmc_float>& salpha_pres,
                                         const Vector<hmc_float>& zeta_pres, const Vector<hmc_float>& beta_pres,
                                         const Vector<hmc_float>& zeta_prev, const Scalar<hmc_float>& sbeta_pres,
                                         const int numeq)
{
    auto out_bufs         = out->get_buffers();
    size_t num_bufs       = out_bufs.size();
    ...
    if (num_bufs != salpha_pres_bufs.size() || num_bufs != zeta_pres_bufs.size() || num_bufs != beta_pres_bufs.size() ||
        num_bufs != zeta_prev_bufs.size() || num_bufs != sbeta_pres_bufs.size()) {
        throw std::invalid_argument("All arguments must use the same number of devices.");
    }

    for (size_t i = 0; i < num_bufs; ++i) {
        auto code = out_bufs[i]->get_device()->getRealCode();
        code->update_alpha_cgm_device(salpha_pres_bufs[i], zeta_pres_bufs[i], beta_pres_bufs[i], zeta_prev_bufs[i],
                                      sbeta_pres_bufs[i], numeq, out_bufs[i]);
    }
}

size_t
physics::lattices::get_flops_update_cgm(const std::string quantity, const int Neqs, const hardware::System& system)
{
