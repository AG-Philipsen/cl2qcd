...
physics::lattices::Staggeredfield_eo::Staggeredfield_eo(
    const hardware::System& system, const StaggeredfieldEoParametersInterface& staggeredfieldEoParametersInterface)
...
physics::lattices::create_staggeredfields_eo(const hardware::System& system, const size_t n,
                                             physics::InterfacesHandler& interfacesHandler)
{
    ...
    for (size_t i = 0; i < n; ++i) {
        fields.push_back(
            new Staggeredfield_eo(system, interfacesHandler.getInterface<physics::lattices::Staggeredfield_eo>()));
    }
...
void physics::lattices::scalar_product(const Scalar<hmc_complex>* res, const Staggeredfield_eo& left,
                                       const Staggeredfield_eo& right)
{
...
void physics::lattices::scalar_product_real_part(const Scalar<hmc_float>* res, const Staggeredfield_eo& left,
                                                 const Staggeredfield_eo& right)
{
    auto res_buffers   = res->get_buffers();
    auto left_buffers  = left.get_buffers();
...
void physics::lattices::sax(const Staggeredfield_eo* out, const Vector<hmc_float>& alpha, const int index_alpha,
                            const Staggeredfield_eo& x)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
    auto x_bufs     = x.get_buffers();
...
void physics::lattices::saxpy(const Staggeredfield_eo* out, const hmc_complex alpha, const Staggeredfield_eo& x,
                              const Staggeredfield_eo& y)
{
    auto out_bufs = out->get_buffers();
    auto x_bufs   = x.get_buffers();
...
void physics::lattices::saxpy(const Staggeredfield_eo* out, const Scalar<hmc_complex>& alpha,
                              const Staggeredfield_eo& x, const Staggeredfield_eo& y)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
    auto x_bufs     = x.get_buffers();
...
void physics::lattices::saxpy(const Staggeredfield_eo* out, const hmc_float alpha, const Staggeredfield_eo& x,
                              const Staggeredfield_eo& y)
{
    auto out_bufs = out->get_buffers();
    auto x_bufs   = x.get_buffers();
...
void physics::lattices::saxpy(const Staggeredfield_eo* out, const Scalar<hmc_float>& alpha, const Staggeredfield_eo& x,
                              const Staggeredfield_eo& y)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
...
void physics::lattices::saxpy(const Staggeredfield_eo* out, const Vector<hmc_float>& alpha, const int index_alpha,
                              const Staggeredfield_eo& x, const Staggeredfield_eo& y)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
    ...
    for (size_t i = 0; i < out_bufs.size(); ++i) {
        auto out_buf = out_bufs[i];
        auto device  = out_buf->get_device();
        device->getSpinorStaggeredCode()->saxpy_eoprec_device(x_bufs[i], y_bufs[i], alpha_bufs[i], index_alpha,
                                                              out_buf);
    }
}

void physics::lattices::saxpby(const Staggeredfield_eo* out, const hmc_complex alpha, const Staggeredfield_eo& x,
                               const hmc_complex beta, const Staggeredfield_eo& y)
{
    auto out_bufs = out->get_buffers();
    auto x_bufs   = x.get_buffers();
    auto y_bufs   = y.get_buffers();
...
void physics::lattices::saxpby(const Staggeredfield_eo* out, const Scalar<hmc_complex>& alpha,
                               const Staggeredfield_eo& x, const Scalar<hmc_complex>& beta, const Staggeredfield_eo& y)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
    auto x_bufs     = x.get_buffers();
    ...
    if (out_bufs.size() != alpha_bufs.size() || out_bufs.size() != beta_bufs.size() ||
        out_bufs.size() != x_bufs.size() || out_bufs.size() != y_bufs.size()) {
        throw std::invalid_argument("Output buffers does not use same devices as input buffers");
    }
    ...
    for (size_t i = 0; i < out_bufs.size(); ++i) {
        auto out_buf = out_bufs[i];
        auto device  = out_buf->get_device();
        device->getSpinorStaggeredCode()->saxpby_eoprec_device(x_bufs[i], y_bufs[i], alpha_bufs[i], beta_bufs[i],
                                                               out_buf);
    }
}

void physics::lattices::saxpby(const Staggeredfield_eo* out, const hmc_float alpha, const Staggeredfield_eo& x,
                               const hmc_float beta, const Staggeredfield_eo& y)
{
    auto out_bufs = out->get_buffers();
    auto x_bufs   = x.get_buffers();
    auto y_bufs   = y.get_buffers();
...
void physics::lattices::saxpby(const Staggeredfield_eo* out, const Scalar<hmc_float>& alpha, const Staggeredfield_eo& x,
                               const Scalar<hmc_float>& beta, const Staggeredfield_eo& y)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
    auto x_bufs     = x.get_buffers();
...
void physics::lattices::saxpby(const Staggeredfield_eo* out, const Vector<hmc_float>& alpha, const int index_alpha,
                               const Staggeredfield_eo& x, const Vector<hmc_float>& beta, const int index_beta,
                               const Staggeredfield_eo& y)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
    ...
    for (size_t i = 0; i < out_bufs.size(); ++i) {
        ...
        auto device  = out_buf->get_device();
        device->getSpinorStaggeredCode()->saxpby_eoprec_device(x_bufs[i], y_bufs[i], alpha_bufs[i], beta_bufs[i],
                                                               index_alpha, index_beta, out_buf);
    }
}

void physics::lattices::saxpbypz(const Staggeredfield_eo* out, const hmc_complex alpha, const Staggeredfield_eo& x,
                                 const hmc_complex beta, const Staggeredfield_eo& y, const Staggeredfield_eo& z)
{
    auto out_bufs = out->get_buffers();
    auto x_bufs   = x.get_buffers();
    auto y_bufs   = y.get_buffers();
...
void physics::lattices::saxpbypz(const Staggeredfield_eo* out, const Scalar<hmc_complex>& alpha,
                                 const Staggeredfield_eo& x, const Scalar<hmc_complex>& beta,
                                 const Staggeredfield_eo& y, const Staggeredfield_eo& z)
{
    auto out_bufs   = out->get_buffers();
    auto alpha_bufs = alpha.get_buffers();
    auto x_bufs     = x.get_buffers();
    ...
    if (out_bufs.size() != alpha_bufs.size() || out_bufs.size() != beta_bufs.size() ||
        out_bufs.size() != x_bufs.size() || out_bufs.size() != y_bufs.size() || out_bufs.size() != z_bufs.size()) {
        throw std::invalid_argument("Output buffers does not use same devices as input buffers");
    }
    ...
    for (size_t i = 0; i < out_bufs.size(); ++i) {
        auto out_buf = out_bufs[i];
        auto device  = out_buf->get_device();
        device->getSpinorStaggeredCode()->saxpbypz_eoprec_device(x_bufs[i], y_bufs[i], z_bufs[i], alpha_bufs[i],
                                                                 beta_bufs[i], out_buf);
    }
}

void physics::lattices::sax_vec_and_squarenorm(const Vector<hmc_float>* res, const Vector<hmc_float>& alpha,
                                               const Staggeredfield_eo& x)
{
...
template<>
size_t
physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, hmc_complex, physics::lattices::scalar_product>(
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, hmc_float,
                                    physics::lattices::scalar_product_real_part>(const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
    auto spinor_code = devices[0]->getSpinorStaggeredCode();
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, physics::lattices::squarenorm>(
    const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, hmc_complex, physics::lattices::sax>(
    const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, hmc_float, physics::lattices::sax>(
    const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, hmc_complex, physics::lattices::saxpy>(
    const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, hmc_float, physics::lattices::saxpy>(
    const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, hmc_complex, physics::lattices::saxpby>(
    const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, hmc_float, physics::lattices::saxpby>(
    const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
...
template<>
size_t physics::lattices::get_flops<physics::lattices::Staggeredfield_eo, physics::lattices::saxpbypz>(
    const hardware::System& system)
{
    // assert single system
    auto devices     = system.get_devices();
