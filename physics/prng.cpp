...
struct PrngFileReader {
    ...
    ~PrngFileReader() { delete[] hostState; }
...
physics::PRNG::PRNG(const hardware::System& system, const physics::PrngParametersInterface* parametersIn)
    ...
    , hPrng(system, parameters->getHostSeed(), parameters->useSameRandomNumbers())
{
    ...
    if (!parameters->getInitialPrngStateFilename().empty()) {
        ...
        for (unsigned int currentBufferIndex = 0; currentBufferIndex < hPrng.get_buffers().size();
             currentBufferIndex++) {
            ...
            if (buffer_bytes != hPrng.get_buffers().at(currentBufferIndex)->get_bytes()) {
                throw std::invalid_argument(parameters->getInitialPrngStateFilename() +
                                            " does not seem to contain a valid prng state");
            }
            hPrng.get_buffers()
                .at(currentBufferIndex)
                ->load(reinterpret_cast<const hardware::buffers::PRNGBuffer::prng_state_t*>(
                    fileContent.prngStates.at(currentBufferIndex).content.get()));
        }
    } else {
...
{
    return physics::buildCheckpointName(parameters->getNamePrefix(), parameters->getNamePostfix(),
                                        parameters->getNumberOfDigitsInName(), number);
}
