...
void physics::set_point_source(const physics::lattices::Spinorfield* spinorfield, int k,
                               const physics::SourcesParametersInterface& params)
{
    ...
    device->getCorrelatorCode()->create_point_source_device(buffer, k,
                                                            Index(params.getSourceX(), params.getSourceY(),
                                                                  params.getSourceZ(), params.getSourceT(),
                                                                  LatticeExtents(params.getNs(), params.getNt()))
                                                                .spatialIndex,
                                                            local_t);
...
static void fill_sources(const std::vector<physics::lattices::Spinorfield*>& sources, const physics::PRNG& prng,
                         const physics::SourcesParametersInterface& params);

std::vector<physics::lattices::Spinorfield*> physics::create_sources(const hardware::System& system, const PRNG& prng,
                                                                     ...
                                                                     physics::InterfacesHandler& interfacesHandler)
{
    ...
    auto sources = lattices::create_spinorfields(system, n_sources, interfacesHandler,
                                                 sourcesParameters.placeSourcesOnHost());
...
std::vector<physics::lattices::Spinorfield*>
physics::create_swappable_sources(const hardware::System& system, const PRNG& prng, const size_t n_sources,
                                  physics::InterfacesHandler& interfacesHandler)
{
    const physics::SourcesParametersInterface& sourcesParameters = interfacesHandler.getSourcesParametersInterface();

    auto sources = lattices::create_swappable_spinorfields(system, n_sources, interfacesHandler,
                                                           sourcesParameters.placeSourcesOnHost());
    fill_sources(sources, prng, sourcesParameters);
    return sources;
}
...
void physics::set_volume_source(const physics::lattices::Staggeredfield_eo* inout, const PRNG& prng)
{
    ...
    for (size_t i = 0; i < buffers.size(); ++i) {
        ...
        buffer->get_device()->getCorrelatorStaggeredCode()->create_volume_source_stagg_eoprec_device(buffer,
                                                                                                     prng_buffer);
    }
...
void physics::set_point_source(const physics::lattices::Staggeredfield_eo* source, int k,
                               const physics::SourcesParametersInterface& params)
{
    if (k > 2 || k < 0) {
    ...
    device->getCorrelatorStaggeredCode()
        ->create_point_source_stagg_eoprec_device(buffer, k,
                                                  Index(params.getSourceX(), params.getSourceY(), params.getSourceZ(),
                                                        params.getSourceT(),
                                                        LatticeExtents(params.getNs(), params.getNt()))
                                                      .spatialIndex,
                                                  local_t);
    ...
    if (buffers.size() != 1) {
        throw Print_Error_Message("Point source for staggered fermions not implemented for multiple devices!", __FILE__,
                                  __LINE__);
        source->update_halo();
    }
}

static void fill_sources(const std::vector<physics::lattices::Staggeredfield_eo*>& sources, const physics::PRNG& prng,
                         const physics::SourcesParametersInterface& params)
{
    using namespace physics;
...
physics::create_staggered_sources(const hardware::System& system, const PRNG& prng, const size_t numberOfSources,
                                  physics::InterfacesHandler& interfacesHandler)
{
    const physics::SourcesParametersInterface& sourcesParameters = interfacesHandler.getSourcesParametersInterface();
    ...
    for (size_t i = 0; i < numberOfSources; ++i) {
        sources.push_back(
            new physics::lattices::Staggeredfield_eo(system,
                                                     interfacesHandler
