...
void physics::set_point_source(const physics::lattices::Spinorfield * spinorfield, int k, const physics::SourcesParametersInterface& params)
{
	...
	device->getCorrelatorCode()->create_point_source_device(buffer, k, get_source_pos_spatial(params.getSourceX(), params.getSourceY(), params.getSourceZ(), params.getNt(), params.getNs()), local_t);
