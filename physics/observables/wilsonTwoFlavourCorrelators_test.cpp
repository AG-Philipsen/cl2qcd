...
void test_correlator(const char* params[], const std::vector<hmc_float>& ps_ref, const std::vector<hmc_float>& sc_ref,
                     const std::vector<hmc_float>& vx_ref, const std::vector<hmc_float>& vy_ref,
                     const std::vector<hmc_float>& vz_ref, const std::vector<hmc_float>& ax_ref,
                     const std::vector<hmc_float>& ay_ref, const std::vector<hmc_float>& az_ref);

BOOST_AUTO_TEST_CASE(point_source_0)
{
    using namespace physics::lattices;

    const char* params[] = {"foo", "--sourceType=point", "--correlatorDirection=0", "--measureCorrelators=1"};
    ...
    hmc_float sc_tmp[] = {0.0024612863844851191, -0.016374834112823547, -0.043961011270704092, -0.01523594115058316,
                          -0.005165987427987663, 0.024049710560252593,  0.061298733087266286,  -0.045272255628473722};
    std::vector<hmc_float> sc(sc_tmp, sc_tmp + sizeof(sc_tmp) / sizeof(hmc_float));
    hmc_float vx_tmp[] = {4.5356982259415997, 4.4780444134285631, 4.4472419503312492, 4.5430446606989898,
                          4.5084478219269606, 4.4981626419407812, 4.5664936387672315, 4.5587559044286214};
    std::vector<hmc_float> vx(vx_tmp, vx_tmp + sizeof(vx_tmp) / sizeof(hmc_float));
    hmc_float vy_tmp[] = {-0.01289411252205299,  -0.03919045962063928, -0.010173642800255882, 0.022418793182812542,
                          -0.045457750951185025, 0.024744535419928094, 0.022423311850699623,  0.028991528724414503};
    std::vector<hmc_float> vy(vy_tmp, vy_tmp + sizeof(vy_tmp) / sizeof(hmc_float));
    hmc_float vz_tmp[] = {0.010392724423067302,  -0.0092227981570463906, -0.0019124537869479354, 0.0015147185464041528,
                          -0.039742353575948297, 0.034283909445848255,   0.015519562913546912,   0.031784190562001421};
    std::vector<hmc_float> vz(vz_tmp, vz_tmp + sizeof(vz_tmp) / sizeof(hmc_float));
    hmc_float ax_tmp[] = {0.031118718400293559,  -0.021908426433681693, -0.041120764054587693, -0.0372499095141723,
                          0.0091808477678250507, 0.039072907858853943,  0.06206377296194393,   -0.045106698264689667};
    std::vector<hmc_float> ax(ax_tmp, ax_tmp + sizeof(ax_tmp) / sizeof(hmc_float));
    hmc_float ay_tmp[] = {-0.029066752308006504,  0.05616977588026148,  -0.027229003286407247, -0.00050158467927097217,
                          -0.0066461981729357446, 0.019173477490037887, -0.033509624999394916, -0.080189233267604784};
    std::vector<hmc_float> ay(ay_tmp, ay_tmp + sizeof(ay_tmp) / sizeof(hmc_float));
    hmc_float az_tmp[] = {-0.047639872928628396,   0.093974382841054555, -0.043026595394032315, -0.019346309887679554,
                          -0.00026516772944583378, 0.020659821224574604, -0.038190332787656045, -0.071688867318142765};
...
BOOST_AUTO_TEST_CASE(point_source_3)
{
    ...
    const char* params[] = {"foo", "--sourceType=point", "--correlatorDirection=3", "--measureCorrelators=1"};
...
BOOST_AUTO_TEST_CASE(stochastic_source_0)
{
    ...
    const char* params[] = {"foo", "--sourceType=volume", "--correlatorDirection=0", "--measureCorrelators=1"};
...
BOOST_AUTO_TEST_CASE(stochastic_source_3)
{
    ...
    const char* params[] = {"foo", "--sourceType=volume", "--correlatorDirection=3", "--measureCorrelators=1"};
...
void check_correlator(std::string which, const std::vector<physics::lattices::Spinorfield*>& solved,
                      const std::vector<physics::lattices::Spinorfield*>& sources, const hardware::System& system,
                      const std::vector<hmc_float>& ref, physics::InterfacesHandler& interfacesHandler)
{
    using namespace std;
...
void test_correlator(const char* _params[], const std::vector<hmc_float>& ps_ref, const std::vector<hmc_float>& sc_ref,
                     const std::vector<hmc_float>& vx_ref, const std::vector<hmc_float>& vy_ref,
                     const std::vector<hmc_float>& vz_ref, const std::vector<hmc_float>& ax_ref,
                     const std::vector<hmc_float>& ay_ref, const std::vector<hmc_float>& az_ref)
{
