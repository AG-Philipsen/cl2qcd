...
class gaugeObservables {
  public:
    ...
    void
    measurePlaqAndPolyAndWriteToFile(const physics::lattices::Gaugefield* gf, int iter, const std::string& filename);
    ...
    physics::observables::Plaquettes
    measurePlaquettes(const physics::lattices::Gaugefield* gaugefield, bool normalize = true);
  ...
  private:
    ...
    void writePlaqAndPolyToFile(int iter, const std::string& filename,
                                const physics::observables::Plaquettes plaquettes, const hmc_complex polyakov);
...
void gaugeObservables::measureGaugeObservablesAndWriteToFile(const physics::lattices::Gaugefield* gaugefield,
                                                             int iteration)
{
                                                        ...
                                                        const std::string& filename)
{
    ...
    if (gaugeObservablesParametersInterface.printToScreen()) {
        logger.info() << iter << '\t' << plaquettes.plaquette << '\t' << plaquettes.temporalPlaquette << '\t'
                      << plaquettes.spatialPlaquette << '\t' << polyakov.re << '\t' << polyakov.im << '\t'
                      << sqrt(polyakov.re * polyakov.re + polyakov.im * polyakov.im);
    }
    writePlaqAndPolyToFile(iter, filename, plaquettes, polyakov);
}

void gaugeObservables::writePlaqAndPolyToFile(int iter, const std::string& filename,
                                              const physics::observables::Plaquettes plaquettes,
                                              const hmc_complex polyakov)
{
...
void gaugeObservables::measureTransportcoefficientKappaAndWriteToFile(const physics::lattices::Gaugefield* gaugefield,
                                                                      int iteration)
{
    hmc_float kappa = kappa_clover(*gaugefield, gaugeObservablesParametersInterface.getBeta());
    writeTransportcoefficientKappaToFile(gaugeObservablesParametersInterface.getTransportCoefficientKappaFilename(),
                                         iteration, kappa);
}
...
physics::observables::Plaquettes
gaugeObservables::measurePlaquettes(const physics::lattices::Gaugefield* gaugefield, bool normalize)
{
    ...
    if (num_devs == 1) {
        ...
        // trigger calculation
        std::vector<const Plain<hmc_float>*> plaqs;
        plaqs.reserve(num_devs);
        std::vector<const Plain<hmc_float>*> tplaqs;
        tplaqs.reserve(num_devs);
        std::vector<const Plain<hmc_float>*> splaqs;
        splaqs.reserve(num_devs);
    ...
    if (normalize) {
        plaquettes.temporalPlaquette /= static_cast<hmc_float>(
            gaugeObservablesParametersInterface.getTemporalPlaquetteNormalization());
        plaquettes.spatialPlaquette /= static_cast<hmc_float>(
            gaugeObservablesParametersInterface.getSpatialPlaquetteNormalization());
...
void physics::observables::measureGaugeObservablesAndWriteToFile(
    const physics::lattices::Gaugefield* gf, int iteration,
...
hmc_float
physics::observables::measurePlaquette(const physics::lattices::Gaugefield* gf,
                                       const physics::observables::GaugeObservablesParametersInterface& parameters)
{
    gaugeObservables obs(parameters);
...
hmc_float physics::observables::measurePlaquetteWithoutNormalization(
...
hmc_float
physics::observables::measureRectangles(const physics::lattices::Gaugefield* gf,
                                        const physics::observables::GaugeObservablesParametersInterface& parameters)
{
    gaugeObservables obs(parameters);
...
hmc_complex
physics::observables::measurePolyakovloop(const physics::lattices::Gaugefield* gf,
                                          const physics::observables::GaugeObservablesParametersInterface& parameters)
{
    gaugeObservables obs(parameters);
...
physics::observables::Plaquettes
physics::observables::measureAllPlaquettes(const physics::lattices::Gaugefield* gf,
                                           const physics::observables::GaugeObservablesParametersInterface& parameters)
{
    gaugeObservables obs(parameters);
