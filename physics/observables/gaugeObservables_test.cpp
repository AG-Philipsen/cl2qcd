...
BOOST_AUTO_TEST_SUITE(PLAQUETTE)
    ...
    class PlaquetteTester : public GaugeObservablesTester {
      public:
        PlaquetteTester(int argc, const char** argv, double referenceValue) : GaugeObservablesTester(argc, argv)
        {
            double testPrecision = 1e-8;
            BOOST_CHECK_CLOSE(physics::observables::measurePlaquette(gaugefield, *gaugeobservablesParameters),
                              referenceValue, testPrecision);
        }
    };
...
BOOST_AUTO_TEST_SUITE(RECTANGLES)
    ...
    class RectanglesTester : public GaugeObservablesTester {
      public:
        RectanglesTester(int argc, const char** argv, double referenceValue) : GaugeObservablesTester(argc, argv)
        {
            double testPrecision = 1e-8;
            BOOST_CHECK_CLOSE(physics::observables::measureRectangles(gaugefield, *gaugeobservablesParameters),
                              referenceValue, testPrecision);
        }
    };
...
BOOST_AUTO_TEST_SUITE(POLYAKOVLOOP)
    ...
    class PolyakovloopTester : public GaugeObservablesTester {
      public:
        PolyakovloopTester(int argc, const char** argv, hmc_complex referenceValue) : GaugeObservablesTester(argc, argv)
        {
            double testPrecision = 1e-8;
...
{
    const char* _params[] = {"foo", "--startCondition=cold", "--nTime=4", "--nSpace=4"};
    meta::Inputparameters parameters(2, _params);
    physics::lattices::GaugefieldParametersImplementation gaugefieldParameters(&parameters);
    physics::observables::GaugeObservablesParametersImplementation gaugeobservablesParameters(parameters);
    hardware::HardwareParametersImplementation hP(&parameters);
