...
// TODO: refactor. Consider also how to handle interfaces (at the moment both the handler and the correlator interfaces
// are given around,
//      but from the first one can get the second). One should do something like in the Wilson pbp class in the .cpp
//      file (namely have as private member the interface of the correlator and as additional argument of some methods
//      the interfacesHandler).
...
static size_t
get_num_corr_entries(const physics::observables::WilsonTwoFlavourCorrelatorsParametersInterface& parametersInterface);
static void
calculate_correlator(const std::string& type, const std::vector<const hardware::buffers::Plain<hmc_float>*>& results,
                     physics::lattices::Spinorfield* corr, physics::lattices::Spinorfield* source,
                     const hardware::System& system,
                     const physics::observables::WilsonTwoFlavourCorrelatorsParametersInterface& parametersInterface,
                     physics::InterfacesHandler& interfacesHandler);
static void calculate_correlator(const std::string& type,
                                 const std::vector<const hardware::buffers::Plain<hmc_float>*>& results,
...
static void flavour_doublet_correlators(
    const std::vector<physics::lattices::Spinorfield*>& result,
    const std::vector<physics::lattices::Spinorfield*>& sources, std::string corr_fn, const hardware::System& system,
    const physics::observables::WilsonTwoFlavourCorrelatorsParametersInterface& parametersInterface,
    physics::InterfacesHandler& interfacesHandler)
{
    using namespace std;
    ...
    for (size_t j = 0; j < result_vx.size(); j++) {
        of << scientific << setprecision(14) << "1 1\t" << j << "\t"
           << (result_vx[j] + result_vy[j] + result_vz[j]) / 3. << "\t" << result_vx[j] << "\t" << result_vy[j] << "\t"
           << result_vz[j] << endl;
    }
    ...
    for (size_t j = 0; j < result_ax.size(); j++) {
        of << scientific << setprecision(14) << "1 0\t" << j << "\t"
           << (result_ax[j] + result_ay[j] + result_az[j]) / 3. << "\t" << result_ax[j] << "\t" << result_ay[j] << "\t"
           << result_az[j] << endl;
    }
                     ...
                     physics::InterfacesHandler& interfacesHandler)
{
    ...
    if (num_bufs > 1 && parametersInterface.getSourceType() != common::sourcetypes::point && type == "ps" &&
        parametersInterface.getCorrelatorDirection() == 3) {
                     ...
                     const physics::observables::WilsonTwoFlavourCorrelatorsParametersInterface& parametersInterface)
{
    ...
    size_t num_bufs = results.size();
    if (num_bufs != source1_bufs.size() || num_bufs != corr1_bufs.size() || num_bufs != source2_bufs.size() ||
        num_bufs != corr2_bufs.size() || num_bufs != source3_bufs.size() || num_bufs != corr3_bufs.size() ||
    ...
    for (size_t i = 0; i < num_bufs; ++i) {
        auto code = results[i]->get_device()->getCorrelatorCode();
        if (parametersInterface.getSourceType() == common::sourcetypes::point) {
            code->correlator(code->get_correlator_kernel(type), results[i], corr1_bufs[i], corr2_bufs[i], corr3_bufs[i],
                             corr4_bufs[i]);
        } else {
            code->correlator(code->get_correlator_kernel(type), results[i], corr1_bufs[i], source1_bufs[i],
                             corr2_bufs[i], source2_bufs[i], corr3_bufs[i], source3_bufs[i], corr4_bufs[i],
                             source4_bufs[i]);
        }
    }
...
static std::vector<hmc_float> calculate_correlator_componentwise(
    const std::string& type, const std::vector<physics::lattices::Spinorfield*>& corr,
...
static std::vector<hmc_float> calculate_correlator_colorwise(
    const std::string& type, const std::vector<physics::lattices::Spinorfield*>& corr,
    ...
    const physics::observables::WilsonTwoFlavourCorrelatorsParametersInterface& parametersInterface)
{
    ...
    for (size_t i = 0; i < corr.size(); i += 4) {
        calculate_correlator(type, results, corr.at(i), sources.at(i), corr.at(i + 1), sources.at(i + 1),
                             corr.at(i + 2), sources.at(i + 2), corr.at(i + 3), sources.at(i + 3), parametersInterface);
    }
...
get_num_corr_entries(const physics::observables::WilsonTwoFlavourCorrelatorsParametersInterface& parametersInterface)
{
    switch (parametersInterface.getCorrelatorDirection()) {
        ...
        default:
            ...
            errmsg << "Correlator direction " << parametersInterface.getCorrelatorDirection()
...
physics::observables::wilson::calculate_correlator(const std::string& type,
                                                   const std::vector<physics::lattices::Spinorfield*>& corr,
                                                   const std::vector<physics::lattices::Spinorfield*>& sources,
                                                   const hardware::System& system,
                                                   physics::InterfacesHandler& interfacesHandler)
{
    const physics::observables::WilsonTwoFlavourCorrelatorsParametersInterface&
        parametersInterface = interfacesHandler.getWilsonTwoFlavourCorrelatorsParametersInterface();
    ...
    } else if (type == "sc" || type == "vx" || type == "vy" || type == "vz" || type == "ax" || type == "ay" ||
               type == "az") {
...
void physics::observables::wilson::measureTwoFlavourDoubletCorrelatorsOnGaugefieldAndWriteToFile(
    ...
    physics::InterfacesHandler& interfacesHandler)
{
    ...
    const std::vector<physics::lattices::Spinorfield*>
        sources = physics::create_swappable_sources(*system, *prng, parametersInterface.getNumberOfSources(),
                                                    interfacesHandler);
    const std::vector<physics::lattices::Spinorfield*>
        result = physics::lattices::create_swappable_spinorfields(*system, sources.size(), interfacesHandler,
                                                                  parametersInterface.placeSourcesOnHost());
    swap_out(sources);
    swap_out(result);
