...
void physics::algorithms::md_update_gaugefield(const physics::lattices::Gaugefield* const gf,
                                               const physics::lattices::Gaugemomenta& gm, const hmc_float eps)
{
...
void physics::algorithms::md_update_spinorfield(const physics::lattices::Spinorfield* const out,
                                                const physics::lattices::Gaugefield& gf,
                                                const physics::lattices::Spinorfield& orig,
...
void physics::algorithms::md_update_spinorfield(const physics::lattices::Spinorfield_eo* const out,
                                                const physics::lattices::Gaugefield& gf,
                                                const physics::lattices::Spinorfield_eo& orig,
                                                const hardware::System& system,
                                                physics::InterfacesHandler& interfacesHandler,
                                                const physics::AdditionalParameters& additionalParameters)
{
 ...
 */
void physics::algorithms::md_update_spinorfield(const physics::lattices::Rooted_Staggeredfield_eo* out,
                                                const physics::lattices::Gaugefield& gf,
                                                const physics::lattices::Rooted_Staggeredfield_eo& orig,
                                                const hardware::System& system,
                                                physics::InterfacesHandler& interfacesHandler,
                                                const physics::AdditionalParameters& additionalParameters)
{
    logger.debug() << "\tRHMC [UP]:\tupdate SF";
    const physics::algorithms::MolecularDynamicsInterface& parametersInterface = interfacesHandler
                                                                                     .getMolecularDynamicsInterface();
    const physics::fermionmatrix::MdagM_eo fm(system,
                                              interfacesHandler.getInterface<physics::fermionmatrix::MdagM_eo>());

    const unsigned int numberOfPseudofermions = interfacesHandler
                                                    .getInterface<physics::lattices::Rooted_Staggeredfield_eo>()
                                                    .getNumberOfPseudofermions();
    for (unsigned int j = 0; j < numberOfPseudofermions; j++) {
        ...
        for (unsigned int i = 0; i < out->getOrder(); i++)
            X.emplace_back(
                std::make_shared<physics::lattices::Staggeredfield_eo>(system,
                                                                       interfacesHandler.getInterface<
        ...
        const int iterations = physics::algorithms::solvers::cg_m(X, fm, gf, out->get_b(), *orig[j], system,
                                                                  interfacesHandler,
                                                                  parametersInterface.getSolverPrec(),
 ...
 */
template<class FERMIONMATRIX, class FERMIONMATRIX_CONJ, class FERMIONMATRIX_HERM, class SPINORFIELD>
static void
md_update_spinorfield_mp(const SPINORFIELD* const out, const physics::lattices::Gaugefield& gf, const SPINORFIELD& orig,
                         const hardware::System& system, physics::InterfacesHandler& interfacesHandler)
{
    ...
    const physics::AdditionalParameters& additionalParameters = interfacesHandler.getAdditionalParameters<SPINORFIELD>(
        false);
    const physics::AdditionalParameters& additionalParametersMp = interfacesHandler
                                                                      .getAdditionalParameters<SPINORFIELD>(true);
    ...
    try {
        ...
        physics::algorithms::solvers::bicgstab(out, qplusMp, gf, temporarySpinorfield, system, interfacesHandler,
                                               parametersInterface.getSolverPrec(), additionalParametersMp);
    }  // try
        ...
        physics::algorithms::solvers::cg(&tmp2, fm_herm, gf, temporarySpinorfield, system, interfacesHandler,
                                         parametersInterface.getSolverPrec(), additionalParametersMp);
...
void physics::algorithms::md_update_spinorfield_mp(const physics::lattices::Spinorfield* const out,
                                                   const physics::lattices::Gaugefield& gf,
                                                   const physics::lattices::Spinorfield& orig,
                                                   const hardware::System& system,
                                                   physics::InterfacesHandler& interfacesHandler)
...
void physics::algorithms::md_update_spinorfield_mp(const physics::lattices::Spinorfield_eo* const out,
                                                   const physics::lattices::Gaugefield& gf,
                                                   const physics::lattices::Spinorfield_eo& orig,
                                                   const hardware::System& system,
                                                   physics::InterfacesHandler& interfacesHandler)
...
static void md_update_gaugemomentum(const physics::lattices::Gaugemomenta* const inout, hmc_float eps,
...
void physics::algorithms::md_update_gaugemomentum(const physics::lattices::Gaugemomenta* const inout, hmc_float eps,
                                                  const physics::lattices::Gaugefield& gf,
                                                  const physics::lattices::Spinorfield& phi,
                                                  const hardware::System& system,
...
void physics::algorithms::md_update_gaugemomentum(const physics::lattices::Gaugemomenta* const inout, hmc_float eps,
                                                  const physics::lattices::Gaugefield& gf,
                                                  const physics::lattices::Rooted_Staggeredfield_eo& phi,
                                                  const hardware::System& system,
                                                  physics::InterfacesHandler& interfaceHandler,
                                                  const physics::AdditionalParameters& additionalParameters)
{
...
// void physics::algorithms::md_update_gaugemomentum(const physics::lattices::Gaugemomenta * const inout, hmc_float eps,
// const physics::lattices::Gaugefield& gf,
//                                                  const physics::lattices::Rooted_Staggeredfield_eo& phi, const
//                                                  hardware::System& system, physics::InterfacesHandler&
//                                                  interfaceHandler, const physics::AdditionalParameters&
//                                                  additionalParameters)
//{
//    using namespace physics::algorithms;
//
...
void physics::algorithms::md_update_gaugemomentum_gauge(const physics::lattices::Gaugemomenta* const gm,
                                                        const hmc_float eps, const physics::lattices::Gaugefield& gf,
                                                        const hardware::System& system,
                                                        physics::InterfacesHandler& interfaceHandler)
{
    const physics::lattices::Gaugemomenta force(system,
                                                interfaceHandler.getInterface<physics::lattices::Gaugemomenta>());
    force.zero();
...
template<class SPINORFIELD>
static void
md_update_gaugemomentum_fermion(const physics::lattices::Gaugemomenta* const inout, hmc_float eps,
                                const physics::lattices::Gaugefield& gf, const SPINORFIELD& phi,
                                const hardware::System& system, physics::InterfacesHandler& interfacesHandler,
                                const physics::AdditionalParameters& additionalParameters)
{
...
void physics::algorithms::md_update_gaugemomentum_fermion(const physics::lattices::Gaugemomenta* const inout,
                                                          hmc_float eps, const physics::lattices::Gaugefield& gf,
                                                          const physics::lattices::Spinorfield& phi,
                                                          const hardware::System& system,
                                                          physics::InterfacesHandler& interfaceHandler,
                                                          const physics::AdditionalParameters& additionalParameters)
...
// void physics::algorithms::md_update_gaugemomentum_fermion(const physics::lattices::Gaugemomenta * const inout,
// hmc_float eps,
//                                                          const physics::lattices::Gaugefield& gf, const
//                                                          physics::lattices::Rooted_Staggeredfield_eo& phi, const
//                                                          hardware::System& system, physics::InterfacesHandler&
//                                                          interfaceHandler, const physics::AdditionalParameters&
//                                                          additionalParameters)
//{
...
//
//    const physics::lattices::Gaugemomenta force(system,
//    interfaceHandler.getInterface<physics::lattices::Gaugemomenta>()); force.zero(); calc_fermion_force(&force, gf,
//    phi, system, interfaceHandler, mass); log_squarenorm("\tRHMC [UP]:\tFORCE [DET]:\t", force); logger.debug() <<
//    "\tRHMC " << "[UP]:\tupdate GM [" << eps << "]"; physics::lattices::saxpy(inout, -1. * eps, force);
//}

template<class SPINORFIELD>
static void
md_update_gaugemomentum_detratio(const physics::lattices::Gaugemomenta* const inout, hmc_float eps,
...
void physics::algorithms::md_update_gaugemomentum_detratio(const physics::lattices::Gaugemomenta* const inout,
                                                           hmc_float eps, const physics::lattices::Gaugefield& gf,
                                                           const physics::lattices::Spinorfield& phi,
                                                           const hardware::System& system,
                                                           physics::InterfacesHandler& interfaceHandler)
