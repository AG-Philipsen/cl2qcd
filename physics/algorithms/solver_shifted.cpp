...
{
    physics::algorithms::solvers::SolverShifted<physics::lattices::Staggeredfield_eo,
                                                physics::fermionmatrix::Fermionmatrix_stagg_eo>
...
physics::algorithms::solvers::SolverShifted<FERMIONFIELD, FERMIONMATRIX>::SolverShifted(
    const std::vector<std::shared_ptr<FERMIONFIELD>> xIn, const FERMIONMATRIX& AIn,
    const physics::lattices::Gaugefield& gfIn, const std::vector<hmc_float> sigmaIn, const FERMIONFIELD& bIn,
    const hardware::System& systemIn, physics::InterfacesHandler& interfacesHandlerIn, hmc_float prec,
    ...
    , b(bIn)
    , system(systemIn)
    , solverPrecision(prec)
    , additionalParameters(additionalParametersIn)
    ...
    , single_eq_resid_host{nullptr}
{
    ...
    if (parametersInterface.getUseMergeKernelsSpinor() == true) {
        single_eq_resid = std::unique_ptr<physics::lattices::Vector<hmc_float>>{
            new physics::lattices::Vector<hmc_float>(numberOfEquations, system)};
    ...
    if (MINIMUM_ITERATIONS)
        logger.warn() << "Minimum iterations set to " << MINIMUM_ITERATIONS
                      << " -- should be used *only* for CGM benchmarking!";
}
...
{
    physics::lattices::update_zeta_cgm(&zeta_foll, zeta, zeta_prev, beta_scalar_prev, beta_scalar, alpha_scalar_prev,
                                       shift, numberOfEquations);
    physics::lattices::update_beta_cgm(&beta_vec, beta_scalar, zeta_foll, zeta, numberOfEquations);
    physics::lattices::update_alpha_cgm(&alpha_vec, alpha_scalar, zeta_foll, beta_vec, zeta, beta_scalar,
                                        numberOfEquations);
}

template<typename FERMIONFIELD, typename FERMIONMATRIX>
void physics::algorithms::solvers::SolverShifted<FERMIONFIELD, FERMIONMATRIX>::updateSingleFieldOfSolution(
    unsigned int index)
{
...
template<typename FERMIONFIELD, typename FERMIONMATRIX>
void physics::algorithms::solvers::SolverShifted<FERMIONFIELD, FERMIONMATRIX>::updateSingleFieldOfAuxiliaryFieldPs(
    unsigned int index)
{
...
template<typename FERMIONFIELD, typename FERMIONMATRIX>
void physics::algorithms::solvers::SolverShifted<FERMIONFIELD, FERMIONMATRIX>::calculateResiduumSingleEquation(
    bool useMergedSpinorKernels, unsigned int index)
{
...
template<typename FERMIONFIELD, typename FERMIONMATRIX>
void physics::algorithms::solvers::SolverShifted<FERMIONFIELD, FERMIONMATRIX>::checkIfSingleEquationConverged(
    unsigned int index)
{
    if (iterationNumber % parametersInterface.getCgIterationBlockSize() == 0) {
        if ((!parametersInterface.getUseMergeKernelsSpinor() && (squarenorm(v) < solverPrecision)) ||
            ...
            logger.debug() << " ===> System number " << index << " converged after " << iterationNumber
                           << " iterations! resid = " << tmp2.get();
        }
    }
}

template<typename FERMIONFIELD, typename FERMIONMATRIX>
bool physics::algorithms::solvers::SolverShifted<FERMIONFIELD, FERMIONMATRIX>::hasSingleSystemConverged(
    unsigned int index)
{
...
{
    logger.debug() << createLogPrefix() << "Solver converged in " << iterationNumber << " iterations! resid:\t"
                   << residuumValue;
    if (logger.beInfo()) {
        ...
        cl_ulong flops_per_iter_no_inner_loop = matrix_flops +
                                                2 * physics::lattices::get_flops<
                                                        physics::lattices::Staggeredfield_eo, hmc_float,
                                                        physics::lattices::scalar_product_real_part>(system) +
                                                3 +
                                                2 * physics::lattices::get_flops<physics::lattices::Staggeredfield_eo,
                                                                                 hmc_float, physics::lattices::saxpy>(
                                                        system) +
                                                physics::lattices::get_flops_update_cgm("alpha", numberOfEquations,
                                                                                        system) +
                                                physics::lattices::get_flops_update_cgm("beta", numberOfEquations,
                                                                                        system) +
                                                physics::lattices::get_flops_update_cgm("zeta", numberOfEquations,
                                                                                        system);
        cl_ulong flops_per_iter_only_inner_loop = physics::lattices::get_flops<physics::lattices::Staggeredfield_eo,
                                                                               hmc_float, physics::lattices::saxpy>(
                                                      system) +
                                                  physics::lattices::get_flops<physics::lattices::Staggeredfield_eo,
                                                                               hmc_float, physics::lattices::saxpby>(
                                                      system) +
                                                  physics::lattices::get_flops<physics::lattices::Staggeredfield_eo,
                                                                               hmc_float, physics::lattices::sax>(
                                                      system) +
                                                  physics::lattices::get_flops<physics::lattices::Staggeredfield_eo,
                                                                               physics::lattices::squarenorm>(system);
        cl_ulong total_flops = iterationNumber * flops_per_iter_no_inner_loop +
                               sum_of_partial_iter * flops_per_iter_only_inner_loop;
        cl_ulong noWarmup_flops = (iterationNumber - 1) * flops_per_iter_no_inner_loop +
                                  (sum_of_partial_iter - numberOfEquations) * flops_per_iter_only_inner_loop;

        logger.debug() << "total_flops: " << total_flops;
        logger.info() << createLogPrefix() << "CG-M completed in " << std::setprecision(6) << duration / 1000.f
                      << " ms @ " << ((hmc_float)total_flops / duration / 1000.f) << " Gflops. Performed "
                      << iterationNumber << " iterations. Performance after warmup: "
    ...
    if (logger.beDebug()) {
        ...
        if (numberOfComponentsToBePrinted > numberOfEquations)
            throw std::invalid_argument(
                "In cg-m numberOfComponentsToBePrinted cannot be bigger than numberOfEquations!");
...
std::string physics::algorithms::solvers::SolverShifted<FERMIONFIELD, FERMIONMATRIX>::createLogPrefix()
{
    ...
    logPrefix << separator_big << label << separator_small << "[" << name << "]" << separator_small << "["
              << strnumber.str() << "]:" << separator_big;
    return logPrefix.str();
}

template<typename FERMIONFIELD, typename FERMIONMATRIX>
void physics::algorithms::solvers::SolverShifted<FERMIONFIELD, FERMIONMATRIX>::debugLogSquarenormSetOfFields(
    const std::string& message, const std::vector<std::shared_ptr<physics::lattices::Staggeredfield_eo>> setOfFields,
    const int reportNumber)
{
    if (logger.beDebug()) {
...
template<typename FERMIONFIELD, typename FERMIONMATRIX>
void physics::algorithms::solvers::SolverShifted<
    FERMIONFIELD, FERMIONMATRIX>::debugCompareSquarenormsOfResultFieldsBetweenBeginAndEndOfIteration()
{
    if (logger.beDebug()) {
        hmc_float tmp;
        logger.debug() << "===============================================";
        for (uint i = 0; i < x.size(); i++) {
            tmp = squarenorm(*x[i]);
            logger.debug() << ((i < 10) ? " " : "") << "delta_sqnorm[field_" << i << "]: " << std::scientific
                           << std::setprecision(16) << tmp - resultSquarenorm[i];
        }
        logger.debug() << "===============================================";
    }
}
...
template<typename FERMIONFIELD, typename FERMIONMATRIX>
const std::vector<std::shared_ptr<FERMIONFIELD>>
physics::algorithms::solvers::SolverShifted<FERMIONFIELD, FERMIONMATRIX>::solve()
{
    if (hasSystemBeSolved)
        return x;
    if (squarenorm(b) == 0) {
    ...
    logger.fatal() << createLogPrefix() << "Solver did not solve in " << parametersInterface.getCgMax()
                   << " iterations. Last resid: " << residuumValue;
