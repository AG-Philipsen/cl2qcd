...
namespace {
    ...
    int cg_singledev(const physics::lattices::Spinorfield_eo* x, const physics::fermionmatrix::Fermionmatrix_eo& f,
    ...
    int cg_multidev(const physics::lattices::Spinorfield_eo* x, const physics::fermionmatrix::Fermionmatrix_eo& f,
                    const physics::lattices::Gaugefield& gf, const physics::lattices::Spinorfield_eo& b,
                    const hardware::System& system, physics::InterfacesHandler& interfacesHandler, hmc_float prec,
                    const physics::AdditionalParameters& additionalParameters);
...
int physics::algorithms::solvers::cg(const physics::lattices::Spinorfield* x,
                                     ...
                                     hmc_float prec, const physics::AdditionalParameters& additionalParameters)
{
    ...
    const physics::algorithms::SolversParametersInterface& parametersInterface = interfacesHandler
                                                                                     .getSolversParametersInterface();
    ...
    // NOTE: here, most of the complex numbers may also be just hmc_floats. However, for this one would need some add.
    // functions...
    for (iter = 0; iter < parametersInterface.getCgMax(); iter++) {
        ...
        // calc residuum
        // NOTE: for beta one needs a complex number at the moment, therefore, this is done with "rho_next" instead of
        // "resid"
        ...
        if (resid < prec) {
            logger.debug() << create_log_prefix_cg(iter) << "Solver converged in " << iter << " iterations! resid:\t"
                           << resid;
            ...
            if (logger.beInfo()) {
                 ...
                 */
                // report performance
                logger.info() << create_log_prefix_cg(iter) << "Solver completed in " << duration / 1000
                              << " ms. Performed " << iter << " iterations";
            }
    ...
    logger.fatal() << create_log_prefix_cg(iter) << "Solver did not solve in " << parametersInterface.getCgMax()
                   << " iterations. Last resid: " << resid;
    throw SolverDidNotSolve(iter, __FILE__, __LINE__);
}

int physics::algorithms::solvers::cg(const physics::lattices::Spinorfield_eo* x,
...
namespace {
    ...
    void reportPerformance_cg(int iter, const uint64_t duration, const uint64_t duration_noWarmup, cl_ulong total_flops,
                              cl_ulong noWarmup_flops, cl_ulong total_bw, cl_ulong noWarmup_bw)
    {
        logger.info() << create_log_prefix_cg(iter) << "CG completed " << iter << " iterations in " << duration / 1000
                      << " ms";
        logger.info() << create_log_prefix_cg(iter) << "Performance [FLOPS]: " << (total_flops / duration / 1000.f)
                      << " GFlops. Performance after warmup: " << (noWarmup_flops / duration_noWarmup / 1000.f)
                      << " Gflops.";
        logger.info() << create_log_prefix_cg(iter) << "Performance [BANDWIDTH]. " << (total_bw / duration * 1e-3)
                      << " GB/s. Performance after warmup: " << (noWarmup_bw / duration_noWarmup * 1e-3) << " GB/s.";
    }

    // todo: move to solvers.hpp
    class Solver {
      public:
        Solver(const hardware::System& systemIn, physics::InterfacesHandler& interfacesHandler,
               const cl_ulong mf_flopsIn, const cl_ulong mf_bwIn)
            : system(systemIn)
            , resid(0.)
            , iter(0)
            , parametersInterface(interfacesHandler.getSolversParametersInterface())
            , RESID_CHECK_FREQUENCY(parametersInterface.getCgIterationBlockSize())
            ...
            , mf_bw(mf_bwIn)
        {
            ...
            if (MINIMUM_ITERATIONS) {
                logger.warn() << "Minimum iterations set to " << MINIMUM_ITERATIONS
                              << " -- should be used *only* for inverter benchmarking!";
            }
        ...
        void reportPerformance(int iter)
        {
            ...
            if (logger.beInfo()) {
                ...
                cl_ulong flops_per_iter = mf_flops + 2 * get_flops<Spinorfield_eo, scalar_product>(system) +
                                          ...
                                          2 * ::get_flops<hmc_complex, complexmult>() +
                                          3 * get_flops<Spinorfield_eo, saxpy>(system);
                cl_ulong flops_per_refresh = mf_flops + get_flops<Spinorfield_eo, saxpy>(system) +
                                             get_flops<Spinorfield_eo, scalar_product>(system);
                ...
                cl_ulong bw_per_iter = mf_bw + 2 * get_read_write_size<Spinorfield_eo, scalar_product>(system) +
                                       2 * ::get_read_write_size<hmc_complex, complexdivide>() +
                                       2 * ::get_read_write_size<hmc_complex, complexmult>() +
                ...
                reportPerformance_cg(iter, duration, duration_noWarmup, total_flops, noWarmup_flops, total_bw,
                                     noWarmup_bw);
            }
        }
    };
                     ...
                     const physics::AdditionalParameters& additionalParameters)
    {
        ...
        // NOTE: here, most of the complex numbers may also be just hmc_floats. However, for this one would need some
        // add. functions...
        int iter = 0;
        for (iter = 0; iter < solver.maximalIterations || iter < solver.MINIMUM_ITERATIONS; iter++) {
            ...
            // rn+1 = rn - alpha*v -> rhat
            // NOTE: for beta one needs a complex number at the moment, therefore, this is done with "rho_next" instead
            // of "resid"
            ...
            if (iter % solver.RESID_CHECK_FREQUENCY == 0) {
                ...
                if (solver.resid < prec && iter >= solver.MINIMUM_ITERATIONS) {
                    ...
                    //  resid_event.wait();
                    //}
                    logger.debug() << create_log_prefix_cg(iter) << "Solver converged in " << iter
                                   << " iterations! resid:\t" << solver.resid;
        ...
        logger.fatal() << create_log_prefix_cg(iter) << "Solver did not solve in " << solver.maximalIterations
                       << " iterations. Last resid: " << solver.resid;
                    ...
                    const physics::AdditionalParameters& additionalParameters)
    {
        ...
        const physics::algorithms::SolversParametersInterface&
            parametersInterface               = interfacesHandler.getSolversParametersInterface();
        ...
        for (iter = 0; iter < parametersInterface.getCgMax(); iter++) {
            ...
            if (parametersInterface.getUseMergeKernelsSpinor()) {
                ...
                // spinor_code->saxpy_AND_squarenorm_eo_device(&clmem_v_eo, &clmem_rn_eo, &clmem_alpha, &clmem_rn_eo,
                // &clmem_rho_next);
                ...
                // calc residuum
                // NOTE: for beta one needs a complex number at the moment, therefore, this is done with "rho_next"
                // instead of "resid"
            ...
            if (resid < prec && iter >= MINIMUM_ITERATIONS) {
                ...
                if (logger.beInfo()) {
                    ...
                    // report performance
                    logger.info() << create_log_prefix_cg(iter) << "CG completed in " << duration / 1000 << " ms @ "
                                  << (total_flops / duration / 1000.f) << " Gflops. Performed " << iter
                                  << " iterations. Performance after warmup: "
