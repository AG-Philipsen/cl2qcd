 ...
 */
static int bicgstab_save(const physics::lattices::Spinorfield* x, const physics::fermionmatrix::Fermionmatrix& A,
...
static int bicgstab_save(const physics::lattices::Spinorfield_eo* x, const physics::fermionmatrix::Fermionmatrix_eo& A,
                         const physics::lattices::Gaugefield& gf, const physics::lattices::Spinorfield_eo& b,
                         const hardware::System& system, physics::InterfacesHandler& interfacesHandler, hmc_float prec,
                         const physics::AdditionalParameters& additionalParameters);

/**
 * BICGstab implementation with a different structure than "save" one, similar to tmlqcd. This should be the default
 * bicgstab. In particular this version does not perform the check if the "real" residuum is sufficiently small!
 */
static int bicgstab_fast(const physics::lattices::Spinorfield* x, const physics::fermionmatrix::Fermionmatrix& A,
                         const physics::lattices::Gaugefield& gf, const physics::lattices::Spinorfield& b,
                         const hardware::System& system, physics::InterfacesHandler& interfacesHandler, hmc_float prec,
                         const physics::AdditionalParameters& additionalParameters);
static int bicgstab_fast(const physics::lattices::Spinorfield_eo* x, const physics::fermionmatrix::Fermionmatrix_eo& A,
                         const physics::lattices::Gaugefield& gf, const physics::lattices::Spinorfield_eo& b,
                         const hardware::System& system, physics::InterfacesHandler& interfacesHandler, hmc_float prec,
                         const physics::AdditionalParameters& additionalParameters);
...
int physics::algorithms::solvers::bicgstab(const physics::lattices::Spinorfield* x,
...
static int bicgstab_save(const physics::lattices::Spinorfield* x, const physics::fermionmatrix::Fermionmatrix& f,
                         const physics::lattices::Gaugefield& gf, const physics::lattices::Spinorfield& b,
                         const hardware::System& system, physics::InterfacesHandler& interfacesHandler, hmc_float prec,
                         const physics::AdditionalParameters& additionalParameters)
{
    ...
    for (iter = 0; iter < parametersInterface.getCgMax(); iter++) {
        ...
        if (resid < prec) {
            ...
            if (trueresid < prec) {
                logger.debug() << create_log_prefix_bicgstab(iter) << "Solver converged in " << iter
                               << " iterations! true resid:\t" << trueresid;
                ...
                if (logger.beInfo()) {
                     ...
                     */
                    // report performance
                    logger.info() << create_log_prefix_bicgstab(iter) << "Solver completed in " << duration / 1000
    ...
    logger.fatal() << create_log_prefix_bicgstab(iter) << "Solver did not solve in " << parametersInterface.getCgMax()
                   << " iterations. Last resid: " << resid;
    throw SolverDidNotSolve(iter, __FILE__, __LINE__);
}

static int bicgstab_fast(const physics::lattices::Spinorfield* x, const physics::fermionmatrix::Fermionmatrix& f,
                         const physics::lattices::Gaugefield& gf, const physics::lattices::Spinorfield& b,
                         const hardware::System& system, physics::InterfacesHandler& interfacesHandler, hmc_float prec,
                         const physics::AdditionalParameters& additionalParameters)
{
...
int physics::algorithms::solvers::bicgstab(const physics::lattices::Spinorfield_eo* x,
...
static int bicgstab_save(const physics::lattices::Spinorfield_eo* x, const physics::fermionmatrix::Fermionmatrix_eo& f,
                         const physics::lattices::Gaugefield& gf, const physics::lattices::Spinorfield_eo& b,
                         const hardware::System& system, physics::InterfacesHandler& interfacesHandler, hmc_float prec,
                         const physics::AdditionalParameters& additionalParameters)
{
        ...
        if (resid < prec) {
            ...
            if (trueresid < prec) {
                ...
                if (logger.beInfo()) {
                    ...
                    cl_ulong total_flops = 4 * get_flops<Spinorfield_eo, scalar_product>(system) +
                                           4 * get_flops<hmc_complex, complexdivide>() +
                                           3 * get_flops<hmc_complex, complexmult>() +
                                           2 * get_flops<Spinorfield_eo, saxsbypz>(system) + 2 * mf_flops +
                    ...
                    total_flops += retests * (mf_flops + get_flops<Spinorfield_eo, saxpy>(system) +
                                              get_flops<Spinorfield_eo, squarenorm>(system));

                    // report performanc
                    logger.info() << create_log_prefix_bicgstab(iter) << "BiCGstab_save completed in "
                                  << duration / 1000 << " ms @ " << (total_flops / duration / 1000.f)
...
static int bicgstab_fast(const physics::lattices::Spinorfield_eo* x, const physics::fermionmatrix::Fermionmatrix_eo& f,
                         const physics::lattices::Gaugefield& gf, const physics::lattices::Spinorfield_eo& b,
                         const hardware::System& system, physics::InterfacesHandler& interfacesHandler, hmc_float prec,
                         const physics::AdditionalParameters& additionalParameters)
{
        ...
        if (resid < prec) {
            ...
            if (logger.beInfo()) {
                ...
                cl_ulong total_flops = get_flops<Spinorfield_eo, squarenorm>(system) + 2 * mf_flops +
                                       4 * get_flops<Spinorfield_eo, scalar_product>(system) +
                                       4 * get_flops<hmc_complex, complexdivide>() +
                                       2 * get_flops<Spinorfield_eo, saxpy>(system) +
                                       2 * get_flops<Spinorfield_eo, saxsbypz>(system) +
                                       3 * get_flops<hmc_complex, complexmult>();
                total_flops *= iter;
                ...
                // report performanc
                logger.info() << create_log_prefix_bicgstab(iter) << "Solver completed in " << duration / 1000
                              << " ms @ " << (total_flops / duration / 1000.f) << " Gflops. Performed " << iter
                              << " iterations";
            }
