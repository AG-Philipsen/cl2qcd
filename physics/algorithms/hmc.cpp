...
template<class SPINORFIELD> static void init_spinorfield_mp(const SPINORFIELD * phi, hmc_float * const spinor_energy_init, const SPINORFIELD * phi_mp,
        hmc_float * const spinor_energy_init_mp, const physics::lattices::Gaugefield& gf, const physics::PRNG& prng, const hardware::System& system);
...
template<class SPINORFIELD> static hmc_observables perform_hmc_step(const physics::lattices::Gaugefield * const gf, const int iter, const hmc_float rnd_number,
        physics::PRNG& prng, const hardware::System& system, physics::InterfacesHandler& interfaceHandler)
{
    ...
    if(params.get_use_mp()) {
        integrator(&new_p, &new_u, phi, *phi_mp.get(), system, interfaceHandler);
    } else {
        ...
        physics::PRNG& prng, const hardware::System& system, physics::InterfacesHandler& interfaceHandler)
{
    ...
    if(params.get_use_eo()) {
        return ::perform_hmc_step<Spinorfield_eo>(gf, iter, rnd_number, prng, system, interfaceHandler);
    } else {
        return ::perform_hmc_step<Spinorfield>(gf, iter, rnd_number, prng, system, interfaceHandler);
    }
}
