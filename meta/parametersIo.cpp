...
meta::ParametersIo::ParametersIo() : options("IO options")
{
    ...
    ("overwriteLastCheckpointEvery", po::value<int>(&savepointfrequency)->default_value(1), "Every how many Markov chain steps the files 'conf.save' and 'prng.save' are updated.")
	("nDigitsInConfCheckpoint", po::value<int>(&config_number_digits)->default_value(5), "The number of digits the the checkpoint filenames.")
