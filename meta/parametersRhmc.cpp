...
meta::ParametersRhmc::ParametersRhmc() : options("RHMC options")
{
    // clang-format off
	options.add_options()
	("rationalApproxOrderHBMD", po::value<unsigned int>(&molecularDynamicsRationalApproximationOrder)->default_value(8), "The order of the rational approximation for the Heat Bath and Molecular Dynamics parts.")
	...
	("findminmaxResidualCheckEvery", po::value<unsigned int>(&findMinMaxEigenvalueIterationBlockSize)->default_value(25), "The frequency at which 'findMinMax' will check the residual.")
	("findminmaxPrecision", po::value<double>(&findMinMaxEigenvaluePrecision)->default_value(1.e-3), "The precision used in 'find_minmax'.")
